...
- fourteen production negative-guard probes on Iran/WebApp:
  ...
  `accountant_market_action`, `tier2_offer_creation`, and
  `tier2_telegram_request`.

With the current manifest count of `5555`, selecting the whole manifest yields
`174` command-plannable scenarios with these drivers:
...
- `14` negative business-guard scenarios with explicit no-partial-mutation
  assertions.
