...
- seventeen production negative-guard probes on Iran/WebApp:
  ...
  `daily_request_limit_exceeded`, and `active_commodity_limit_exceeded`.

With the current manifest count of `5555`, selecting the whole manifest yields
`177` command-plannable scenarios with these drivers:
...
- `17` negative business-guard scenarios with explicit no-partial-mutation
  assertions.
