...
برای export عملیاتی کل تاریخچه، `--temporary-directory` اجباری است. این مسیر باید یک
دایرکتوری خالی با mode برابر `0700`، متعلق به همان کاربر اجرا و روی storage دیسکی کنترل‌شده باشد؛ استفاده از `/tmp`
میزبان وب (که `tmpfs` است) ممنوع است. SQLite فایل sort موقت را فقط در همین مسیر می‌سازد و
پس از بسته‌شدن transaction مسیر باید دوباره خالی باشد. وجود artifact اجرای قبلی باعث
توقف fail-closed می‌شود و نباید خودکار پاک شود.
