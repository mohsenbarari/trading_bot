...
| `BOT-008` | Projection completion atomically triggers the two current educational messages before the panel, then acknowledgements, the one-time welcome message, channel link, WebApp link, and channel-join controls in the exact current order and conditions. Repeat with the Telegram user already a channel member, admin, and owner; no absent `chat_join_request` may bypass the tutorials. After initial completion, plain `/start`, a reopened link token, and a reopened invitation must show the approved active-account channel/WebApp links message without repeating welcome copy. Cover out-of-order and duplicate acknowledgements and recovery through `/start`. |
