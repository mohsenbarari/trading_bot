...
- گزارش بسته‌شدن ۲۰۲۶-۰۷-۱۵ یک snapshot تاریخی از runtime `1a698b8d` است. تغییرات post-review این snapshot را به‌عنوان evidence برای HEAD جدید بی‌اعتبار می‌کند و تا rerun نباید «پذیرش نهایی» تلقی شود.
...
## بازبینی مستقل نهایی و بازگشایی پذیرش - ۲۰۲۶-۰۷-۱۵

چهار گزارش مستقل ChatGPT Pro، ChatGPT Ultra، Claude و Gemini روی `48e82389` بررسی و هر finding با کد و evidence واقعی تطبیق داده شد. نتیجه تفصیلی در `docs/MARKET_16_STAGE_INDEPENDENT_REVIEW_REMEDIATION_20260715.md` ثبت می‌شود. قواعد حاکم بر این بازگشایی:

- Stageهای ۱ تا ۴ از نظر historical staging evidence همواره `PARTIAL` می‌مانند؛ revalidation جدید جای لاگ تاریخی حذف‌شده را نمی‌گیرد.
- تضمین Stage ۴، اعمال اتمیک policy بر داده قابل مشاهده در هر authoritative home است. در partition، quota سراسری قطعی تضمین نمی‌شود و ساخت reservation authority جدید خارج از معماری تأییدشده است.
- تضمین Stage ۶، جلوگیری قطعی از race `create-vs-close` در هر home محلی و convergence بعدی foreign است؛ توقف آنی سراسری هنگام قطع ارتباط ادعا نمی‌شود.
- شواهد mutating دو سرور Stageهای ۵ و ۱۰ هنوز ناقص‌اند. تست واقعی چندپردازه Redis برای Stage ۱۴ به source gate افزوده می‌شود، اما جای smoke واقعی staging با چند API worker را نمی‌گیرد.
- manifest با ۵۶۱۱ ردیف یک catalog پوشش است، نه ۵۶۱۱ اجرای مستقل. تا وجود mapping صریح گروه به evidence و اجرای سناریوهای connectivity/ambiguous retry، عبارت «full release evidence» مجاز نیست.
- هر نتیجه staging قبلی که روی `1a698b8d` اجرا شده برای regression تاریخی مفید است، اما HEAD remediated فقط پس از deploy و rerun محدود و صریح قابل پذیرش staging خواهد بود.
...
هیچ معامله مشتری خارج از policy بر اساس داده و reservationهای قابل مشاهده در authoritative home همان معامله ذخیره نشود و تمام مسیرهای ثبت معامله از یک validation canonical عبور کنند. این معیار quota سراسری و هم‌زمان میان دو home در زمان قطع ارتباط را تضمین نمی‌کند؛ آن حالت ریسک پذیرفته‌شده availability-first است و بعد از sync فقط قابل تشخیص و گزارش است، نه rollback خودکار معامله قطعی.
...
ریسک باقیمانده به نسخه مختلط schema و stale mirror محدود می‌شود. rollout باید migration-first باشد؛ نسخه قدیمی ستون جدید را نادیده می‌گیرد و نسخه جدید در نبود runtime state معتبر fail-closed می‌شود. rollback کد با نگه‌داشتن ستون افزایشی امن است؛ حذف ستون تا پایان rollout و retry window ممنوع است. migration تاریخی `c9a4e7b2d615` ستون و index بدون مصرف runtime به نام `republished_offer_public_id` را نیز نگه داشته است. بازنویسی migration اعمال‌شده ممنوع است؛ حذف این residue فقط با migration افزایشی جدید، پس از drain کامل receipt/retry و تأیید rollout همه runtimeها انجام می‌شود.
...
در هر authoritative home، هیچ interleaving قابل بازتولید `create-vs-close` نتواند پس از نهایی‌شدن close محلی یک آفر فعال جدید ایجاد کند. close سراسری آنی در زمان partition ادعا نمی‌شود؛ foreign پس از دریافت state یا autonomy grace همگرا می‌شود.
...
- رقابت یک Trade درحال commit با batch بستن بازار می‌تواند به‌علت optimistic versioning کل batch close را تا cycle بعدی rollback کند. این رفتار pre-existing، self-healing و خارج از finding `create-vs-close` است؛ تا طراحی جداگانه per-offer close isolation، معیار خروج Stage ۶ نباید trade-vs-close را پوشش‌داده تلقی کند.
...
- انتخاب due-only با ترتیب پایدار oldest-first، retry metadata پایدار و backoff نمایی
...
وضعیت: بسته شد؛ پیاده‌سازی، تست PostgreSQL و پذیرش مشترک staging کامل شده است.
...
وضعیت: بسته شد؛ پیاده‌سازی، تست PostgreSQL، پذیرش pagination در ماتریس staging و viewport سه‌مرورگر کامل شده است.
...
وضعیت: بسته شد؛ پیاده‌سازی، تست PostgreSQL و پذیرش تاریخچه در ماتریس staging کامل شده است.
...
وضعیت: بسته شد؛ پیاده‌سازی، suite کامل realtime و پذیرش دو نقش deploy شده staging کامل شده است.
...
وضعیت: بسته شد؛ پیاده‌سازی، تست PostgreSQL، rollout افزایشی و deep sync parity روی staging کامل شده است.
...
وضعیت: بسته شد؛ پیاده‌سازی و پذیرش نهایی UI/API/sync روی staging کامل شده است.
...
| `MKT-02` | ۴ | رد قطعی معامله مشتری خارج از policy قابل مشاهده در authoritative home محلی |
...
| `MKT-06` | ۶ | نبود create فعال جدید پس از close قطعی همان home و convergence مستند foreign |
...
## گزارش بسته‌شدن نهایی - ۲۰۲۶-۰۷-۱۵

### نسخه و rollout

- runtime نهایی روی staging ایران و خارج با `RELEASE_SHA=1a698b8d6fbbe747d9aed171ff2e171ddded2134` و Alembic head برابر `f1b6e7f8a9dc` اجرا شد. rollout به‌ترتیب receiver-first روی ایران و سپس foreign API، bot و workerهای خارج انجام شد.
- پیش از rollout از هر دو دیتابیس staging پشتیبان گرفته شد. SHA-256 پشتیبان خارج `8d91cb06c80509c495df9450abd0f1e456a3061f5d4896b66f17261fcfd050cc` و پشتیبان ایران `4cbf72a97960ced5326b8349c4bb95fa2dea0f52af88f08ea66afcc5a89c01e4` است.
- preflight نهایی `MARKET-16-STAGE-FINAL-20260715-POSTMATRIX` هر ۳۰ check را پاس کرد: branch و release binding، role isolation، migration، guard سطح عمومی خارج، sync health و deep parity. business drift و critical drift هر دو صفر و پنج اختلاف باقی‌مانده فقط non-business/local بود.
- لاگ ۴۵ دقیقه پس از rollout در `foreign_app`، `bot`، `foreign_sync_worker`، Iran `app` و Iran `sync_worker` فاقد level `ERROR`/`CRITICAL`، traceback و `StaleDataError` بود.

### ماتریس کنترل‌شده بازار

- ماتریس mutation در دیتابیس ایزوله staging شامل ۱۲۰ سناریو و ۴۸۰۰ عملیات بود. هر سناریو دقیقاً ۴۰ attempt داشت و خانواده‌های create، معامله همزمان/غیرهمزمان، انقضای دستی/زمانی، contention و رد معامله پس از completed/manual-expired/time-expired را پوشش داد. نتیجه `120/120` پاس، صفر request error و cleanup موفق بود. SHA-256 artifact برابر `7b221e4b590dfde6e40c41a1de54e20117cc61d2025583e1bfc26629adfb32f8` است.
- ماتریس read در دیتابیس ایزوله staging شامل ۱۰۸ سناریو و ۴۳۲۰ درخواست بود: active market، public detail و market history روی Bot/WebApp و محورهای role/surface. نتیجه `108/108` پاس، صفر request error و cleanup موفق بود. SHA-256 artifact برابر `a6c8690f54d129649672161e18ee1a532097eac0ab4fcaaa484a5c2cd46ef614` است.
- ماتریس trade delivery شامل ۱۰۸ سناریوی executable، ۱۱ actor-pair، چهار surface-pair و سه کلاس outage بود. هر ۱۰۸ مورد پاس شد؛ تماس واقعی تلگرام با gateway تزریقی غیرفعال بود و receipt/notification و رفتار short/medium outage بررسی شد. SHA-256 artifact برابر `e8002145349f18fd747927e15ec34fa7a582400b170a2410a321416643e93161` است.
- شش سناریوی یکتای واقعی دو سرور شامل WebApp/Bot origin، خرید/فروش، عمده/خرد، duplicate replay و race انقضای دستی/زمانی پاس شدند؛ پس از هر اجرا cleanup، deep parity و sync health پاس شد. اجرای نخست سه سناریوی WebApp را پاس کرد و سه سناریوی Bot فقط در مرحله آماده‌سازی test harness به‌علت grammar قدیمی `ف` به‌جای `ف ن` متوقف شدند. runtime parser درست بود؛ harness در commit `1a698b8d` اصلاح و هر سه سناریوی Bot در rerun پاس شدند.
- مجموع ماتریس کنترل‌شده ۳۴۲ سناریوی یکتا بود: ۲۲۸ سناریوی جامع با ۹۱۲۰ درخواست، ۱۰۸ سناریوی تحویل و شش سناریوی واقعی دو سرور. manifest کامل ۵۶۱۱ ردیف دارد، اما این گزارش ادعا نمی‌کند هر ردیف manifest جداگانه mutation شده است؛ پذیرش بر پوشش تمام خانواده‌ها و محورهای حساس تعریف‌شده است، نه تست فشار یا ضرب دکارتی کامل.

### UI، realtime و پاک‌سازی

- test harness مرورگر برای قرارداد جدید `/api/offers/page`، endpoint `my/repeatable` و سه گروه هم‌ردیف فیلتر در commit `3394e518` هم‌راستا شد. Chromium هر ۱۰ تست را پاس کرد. ماتریس Chromium/Firefox/WebKit مقدار `29 passed` و یک failure بدون trace کافی در WebKit/mobile-390 داشت؛ rerun دقیق همان مورد برابر `1 passed` شد. علت اجرای اول اثبات نشده است و به‌عنوان flake احتمالی، نه خطای قطعی navigation، ثبت می‌شود.
- suite کامل `test_realtime*.py` روی final candidate برابر `26 passed` شد و transport اصلی Redis، fallback فقط هنگام failure، dedupe event id، SSE/WebSocket، guard خصوصی و lifecycle manager را پوشش داد.
- هر سه پروژه Docker ایزوله mutation/read/delivery همراه PostgreSQL/Redis volumeهای موقت حذف شدند. fixtureهای هر سناریو و ۶۴ کاربر هر ماتریس جامع نیز توسط cleanup محافظت‌شده حذف شدند.
- production deploy، restart یا mutation انجام نشد. برای این پذیرش نیازی به کپی یا مقایسه داده production نبود؛ PostgreSQLهای scratch و staging-isolated داده کافی و کنترل‌شده فراهم کردند.

### وضعیت انتشار

- گزارش تاریخی roadmap روی runtime `1a698b8d` از نظر توسعه و staging بسته شده بود، اما بازبینی مستقل ۲۰۲۶-۰۷-۱۵ acceptance را برای HEAD جدید بازگشایی کرد. تا اجرای gateهای post-review، این branch نه staging-closed و نه production-merge-ready است.
- commit `3394e518` فقط test harness است و لازم نیست برای تغییر runtime دوباره deploy شود. SHA اجراشده روی staging همان `1a698b8d6fbbe747d9aed171ff2e171ddded2134` باقی می‌ماند.
- هنگام بسته‌شدن، `main@04fef2a5` دو commit از candidate جلوتر است: `bad67541` برای جابه‌پذیرشدن بلوک معامله/تسویه و `04fef2a5` برای بازگرداندن migrationهای deploy‌شده. `git merge-tree` یک conflict واقعی در `bot/handlers/trade_create.py` و هم‌پوشانی معنایی parser/tutorial نشان داد؛ بنابراین merge خودکار امن نیست. پیش از merge نهایی باید این دو commit دستی ادغام و تست‌های parser، bot create، migration smoke و preflight دو سرور دوباره اجرا شوند. این gate به معنی شکست Stageهای آزموده‌شده نیست، اما branch فعلی را بدون آن بازآزمایی برای production merge-ready نمی‌کند.
