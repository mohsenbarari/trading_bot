...
| ۵ | `MKT-04` | republish فقط روی سرور مرجع | بالا | ندارد |
...
| ۹ | `MKT-10` | جلوگیری از گرسنگی صف workerهای ترمیم | بالا | Stage ۵ |
| ۱۰ | `MKT-11` | cancel-all دقیق و مقاوم در رقابت | بالا | Stage ۵ و Stage ۱۱ |
| ۱۱ | `MKT-13` | idempotency انقضای forwarded offer | بالا | Stage ۵ |
...
- mutation آفر قدیمی فقط روی `home_server` همان آفر انجام شود.
- آفر جایگزین مطابق source invariant ساخته شود: WebApp روی Iran و بات روی foreign؛ home آفر قدیمی به آفر جدید تحمیل نشود.
- سرور غیرمرجع فقط command را forward کند و قبل از نتیجه durable، mirror یا آفر جایگزین را تغییر ندهد.
- در قطع ارتباط پیش از commit، وضعیت بدون تغییر بماند. اگر commit سرور مرجع انجام و پاسخ گم شد، retry از receipt همان نتیجه را بازیابی کند.

### محدوده تغییر

- endpoint و service مربوط به republish در `api/routers/offers.py`
- command contract مبتنی بر `offer_public_id`، authority guard، receipt پایدار و sync convergence
- جلوگیری از انتشار یا ویرایش زودهنگام پیام کانال
- lineage آفر قدیمی و جایگزین بدون اتکا به peer-local integer ID

### تصمیم معماری تأییدشده

- برای آفر Iran-home، expire آفر قدیمی، ساخت جایگزین و lineage تا حد ممکن در یک transaction محلی و با `require_authority=True` انجام شود.
- برای آفر foreign-home که از WebApp ایران republish می‌شود، distributed transaction روی HTTP ساخته نمی‌شود. command idempotent روی foreign آفر قدیمی را expire/reserve می‌کند؛ سپس ایران آفر جایگزین Iran-home را با public ID و idempotency ثابت می‌سازد و lineage با reconciliation تکمیل می‌شود.
- receipt/fingerprint عمومی command در این Stage پایه‌گذاری می‌شود، اما Stage ۱۱ استفاده canonical آن را برای تمام expireهای forwarded کامل می‌کند. پیاده‌سازی ledger دوم و موازی در Stage ۱۱ ممنوع است.
- side effectهای Telegram، realtime، counter و publication فقط پس از commit قطعی و با dedupe key پایدار اجرا شوند.

### تست‌های اجباری

- آفر Iran-home از WebApp ایران محلی republish شود.
- آفر foreign-home از ایران فقط روی foreign منقضی و جایگزین آن Iran-home ساخته شود.
- قطع ارتباط، timeout و پاسخ گمشده mirror را تغییر ندهند.
- سناریوی «commit foreign + گم‌شدن پاسخ + restart + retry» دقیقاً یک جایگزین بسازد.
- stale mirror نتواند نسخه جدیدتر home را overwrite کند.
- رقابت republish با معامله/انقضا نتیجه قطعی و سازگار بدهد.
- retry همان فرمان، republish دوم یا پیام کانال دوم نسازد.
- تفاوت local integer ID بین دو سرور با `offer_public_id` درست resolve شود.
- quota، counter و lineage فقط یک‌بار مصرف/ثبت شوند.

### معیار خروج

در تمام سناریوها هر Offer فقط روی home خودش mutation شود، replacement home از source surface تبعیت کند و هر دو سرور نهایتاً روی old/new lineage و terminal state همگرا شوند.
...
- reuse همان receipt/fingerprint foundation ایجادشده برای republish در Stage ۵؛ ledger موازی ساخته نشود
...
- fingerprint باید source/home، owner/actor semantic identity، کالا، جهت، settlement، quantity، lotهای canonical، قیمت، notes policy-relevant و republish target public identity را پوشش دهد.
- Stageهای ۵، ۶، ۷ و ۸ ابتدا semantics نهایی republish، admission/quota و settlement را تثبیت می‌کنند.
- helper fingerprint Stage ۵ باید reuse و version شود؛ الگوریتم دوم برای Offer create ساخته نشود.
...
| `MKT-04` | ۵ | mutation هر Offer فقط روی home خودش، replacement مطابق source و lineage همگرا |
