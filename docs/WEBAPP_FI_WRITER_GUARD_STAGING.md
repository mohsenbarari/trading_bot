...
preflight binds all of the following to one signed Release-0 descriptor:

- the fixed legacy application release `2c08da14bfa0ef94d9c788e478d30ddc3f31a3c5`;
- clean Git checkouts whose application/control commit and tree IDs equal the
  signed identity, plus the exact fenced Compose bytes;
- the preloaded app and bot repository digests and Docker image IDs;
...
The signed descriptor and its authority key are installation inputs, not
...
contains secrets and stays root-only.  The identity descriptor, authority
public key, and expected descriptor SHA-256 are separate root-owned inputs.
