...
سیستم باید بتواند در زمان پیک حداقل `3` آفر در ثانیه را بدون ازدست‌دادن رکورد بپذیرد، هر آفر واجد انتشار را پیش از deadline کسب‌وکار به‌صورت یک پیام مستقل در همان کانال فعلی منتشر کند و خطاهای موقت تلگرام را بدون نمایش وضعیت غیرعادی به کاربر مدیریت کند. اگر این هدف با یک بات/یک کانال و عمر فعلی آفر در staging محقق نشود، نتیجه release برابر `NO-GO` است؛ صف مجاز نیست شکست ظرفیت را با انتشار stale، تغییر پنهانی expiry یا اعلام موفقیت کاذب بپوشاند.
...
5. اگر staging این گیت را با یک بات/یک کانال رد کند، نتیجه `NO-GO` است. expiry، SLO، کانال، batching یا ناشر دوم بدون تصمیم تازه مالک محصول تغییر نمی‌کند و سیستم با semantics ضعیف‌تر deploy نمی‌شود.
...
| مالک Operations/on-call | credential، staging pool، dashboard، pause/resume و incident command | دریافت alert حداکثر ۱ دقیقه و acknowledge حداکثر ۱۰ دقیقه |
...
| `N07–N18` | Operations + Security | Stage 1، 4، 5 و 6 | توقف تست/release، حفظ artifact redacted و عدم لمس production |
