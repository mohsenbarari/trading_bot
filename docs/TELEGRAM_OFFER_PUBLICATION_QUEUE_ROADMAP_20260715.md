...
- وضعیت authoritative پس از remediation review در `2026-07-19`: یافته‌های `TQ-F002..TQ-F006`, `TQ-F008..TQ-F010` روی successor کامیت `78df2e14` اصلاح یا به گیت دقیق زنده تفکیک شده‌اند؛ `TQ-F001` به‌عنوان P1 اعشاری رد و فقط hardening قرارداد رسمی Integer پذیرفته شد؛ `TQ-F007` از نظر وجود تناقض منطقی رد شد، ولی ساختار سند برای حذف امکان بدخوانی اصلاح شد؛ `TQ-F011..TQ-F012` گیت زنده Stage 4 باقی می‌مانند. runtime همچنان `TELEGRAM_DELIVERY_QUEUE_IMPLEMENTATION_READY=False` است. Stage 3 تا ساخت evidence exact-SHA و re-review مستقل «مرج/اجرای زنده مجاز» محسوب نمی‌شود.
