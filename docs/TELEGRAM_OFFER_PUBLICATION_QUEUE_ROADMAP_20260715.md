...
- freshness سایر actionها شامل callback، success خصوصی آفر، درخواست و اعلان معامله، پیام مدیریتی، وضعیت بازار و cleanup زمان‌دار، به‌همراه تکمیل coverage router هر lane، feeder cutover، مسیر عملیاتی pause/resume و reconcile jobهای blocked باید در ادامه Stage 3 تکمیل شوند. OTP خارج shared queue باقی می‌ماند.
