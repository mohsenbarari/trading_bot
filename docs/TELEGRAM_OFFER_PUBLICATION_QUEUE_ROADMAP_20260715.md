...
- مبنای تاریخی شاخه: `main@ca6348af`؛ merge-base جاری با `origin/main@2c08da14` برابر `2d7c114d` است.
...
- checkpoint هشتم cutover پیام‌ساز interaction در `2026-07-18`: سیزده پاسخ مستقل flow دعوت مشتری سطح یک، شامل gate همگام‌سازی، prompt و validation نام/موبایل، state ناقص، خطای forwarding، انتظار projection و لغو، به adapterهای پیام ورودی/callback منتقل شدند. inline keyboardهای موقت در metadata قرارداد علامت‌گذاری می‌شوند و ترتیب فعلی mutationهای FSM حفظ شده است. پیام نتیجه نهایی دعوت عمداً مستقیم باقی ماند، چون بلافاصله پس از آن پنل موجود edit می‌شود و بدون dependency/order contract ممکن است ترتیب قابل مشاهده معکوس شود. `22` تست کامل panel متمرکز و `573` تست گسترده Telegram با `152` skip پاس شدند؛ inventory جاری total=`395` و `remaining_interactive_direct=291` است.
...
- checkpoint کنترل‌پلین پایدار `2026-07-19`: 429 پیش‌پرواز و pauseهای سخت bot/gateway پیش از sleep یا feedback در جدول foreign-local پایدار ثبت می‌شوند و claim/final marker هر دو آن‌ها را enforce می‌کنند. resume بات و gateway یک saga دارای advisory fence، full permission preflight، journal فازهای DB/Redis، request-id idempotent، actor هش‌شده، شناسه jobهای پایدار و CLI break-glass دارد. قطع پس از تغییر DB و پیش از Redis با همان request بدون دوباره‌آزادکردن job ادامه می‌یابد. migration `fd29c0e1f3a4` رفت‌وبرگشت واقعی، پنج تست PostgreSQL fault/race/restart، ۹ تست resume مقصد و ۵۵ تست unit/schema/CLI/sync پاس شدند؛ runtime و deploy همچنان خاموش‌اند.
- checkpoint زمان و ترتیب پایدار `2026-07-19`: limiter دیگر هیچ comparison اتمیک admission/cooldown/preflight را از ساعت process نمی‌گیرد و Lua با `Redis TIME` کار می‌کند؛ skew یک‌ساله و ۵۰ contender هم‌زمان نتوانستند cadence یا cooldown را دور بزنند. برای edit آفر، `source_order_at` بادوام از `Offer.created_at` به job افزوده شد تا newest-first در کل backlog و میان دورهای feeder برقرار باشد؛ fresh/stale پنج‌دقیقه‌ای و catch-up یک‌در‌بیست نیز در comparator claim اعمال می‌شوند. migration `fe30d1e2f4b5` رفت‌وبرگشت واقعی، `31` تست limiter/Redis، سه depth test PostgreSQL و کل `64` تست queue PostgreSQL پاس شدند؛ runtime و deploy خاموش‌اند.
- checkpoint تاریخی closure در `2026-07-19` روی `78df2e14`: در آن snapshot کار source-local Stage 3 بسته اعلام شد؛ این ادعا پس از review مستقل ChatGPT Pro دوباره باز شد و دیگر وضعیت جاری یا authoritative نیست. اعداد و عبارت‌های «Stage 3 باز/ناکامل/بسته» در تمام checkpointهای تاریخی فقط وضعیت همان snapshot را شرح می‌دهند و هرگز بر جدول واحد زیر مقدم نیستند.
- checkpoint نهایی cutover و سخت‌سازی محلی `2026-07-19`: actionهای `preauth_interaction` و `preauth_interaction_edit` به migration افزایشی `a274f5a6b8c9` و outbox زمان‌دار افزوده شدند؛ پیام‌های private و non-private قبل از auth، editهای وابسته به `message_id` واقعی provider و `sendDocument` حداکثر `5 MiB` با payload بادوام، hash، multipart gateway، retention/redaction و ambiguity fail-closed پوشش داده شدند. timerهای suggestion در queue mode دیگر detached نیستند. بازاجرای گسترده `698` تست Telegram و `541` تست Bot بدون skip، ResourceWarning، unraisable exception، خطای loop یا رشد file descriptor پاس شد؛ PostgreSQL queue/resume/rollback `78`، limiter واقعی Redis `17` و تکرار probe `30/30` نیز سبز است. runner هشت تست ایمنی دارد، raw log و JSON را مستقل ثبت می‌کند و هر اجرای evidence با PostgreSQL session advisory lock مالک یکتای همان DB/Redis scratch می‌شود؛ runner موازی پیش از discovery یا هر test side effect به‌صورت fail-closed رد و اجرای معتبر در PTY پایدار تا exit واقعی نگه داشته می‌شود. یک نقص واقعی privacy در relink کشف و اصلاح شد: immutable Telegram audience پیش از هر reclassification/version check سنجیده می‌شود و outboxهای trade/admin/account هیچ intent قدیمی را به route جدید بازهدف‌گذاری نمی‌کنند. provider fact قطعی نیز مستقل از feedback باقی می‌ماند و پس از restart بدون provider call دوم replay می‌شود.
- checkpoint reconciliation نهایی `2026-07-19`: `origin/main=2c08da14`، merge-base=`2d7c114d` و divergence برابر `99/7` است؛ `git cherry -v HEAD origin/main` هر هفت commit فعلی main را با علامت `-` یعنی patch-equivalent در candidate گزارش می‌کند. بنابراین merge/rebase اضافی نه لازم است و نه بدون تأیید مالک مجاز. هر تغییر آینده main این checkpoint را باطل و `R26` را دوباره باز می‌کند.
- وضعیت authoritative پس از remediation review در `2026-07-19`: بازبینی مستقل successor `a1a94c1a` هشت finding جدید `TQ-R001..TQ-R008` را باز کرد. همه به‌عنوان blocker ثبت و در successor بعدی به‌ترتیب runtime (`R001..R003`)، اعتبار evidence و isolation (`R004..R006`) و کامل‌بودن سناریو/fault (`R007..R008`) اصلاح شدند؛ disposition و شاهد دقیق در بخش `13.15` است. `TQ-F011..TQ-F012` همچنان فقط گیت زنده Stage 4 هستند. runtime همچنان `TELEGRAM_DELIVERY_QUEUE_IMPLEMENTATION_READY=False` است و تا evidence تمیز exact-SHA و re-review مستقل جدید، merge، اجرای زنده staging و deploy مجاز نیست.
- checkpoint جاری successor بازبینی `ef15604d` در `2026-07-19`: گزارش مستقل پنج blocker جدید `EF-RV-001..005` را اثبات کرد و وضعیت قبلی بخش `13.15` را supersede نمود. اصلاح source-local هر پنج مورد در بخش `13.16` ثبت شده است: مالک یکتای میان‌پردازه‌ای worker، trace سازگار و state machine خام، obligation مبتنی بر outcome قطعی، مرز امضاشده و OS-level میان sender/observer، scanner با inventory دقیق و trust root ثابت/host-bound/یک‌بارمصرف. این checkpoint تا تست‌های کامل و bundle exact-SHA فقط `REMEDIATED-LOCAL / RE-REVIEW-REQUIRED` است؛ Stage 4 زنده، merge و هر deploy همچنان ممنوع‌اند.
...
### نمای authoritative Stageها

| Stage | وضعیت جاری | مرز خروج/گام بعد |
| --- | --- | --- |
| `0` | `CLOSED` | قرارداد، شاخه candidate و roadmap ثبت شده‌اند. |
| `1` | `CLOSED` | سناریوی seedدار `1800 valid + 400 invalid`، ماتریس پاسخ طبیعی و فنی، cleanup و artifact تعریف شده‌اند. |
| `2` | `CLOSED-LOCAL` | قراردادهای pure، failure injection، priority/freshness/retry/ambiguity و regression محلی سبزند. |
| `2.5` | `CLOSED` | challenge register و ADRها بدون ابهام طراحی باز هستند؛ owner، rollback و شاهد هر مورد ثبت شده است. |
| `3` | `REMEDIATED-LOCAL / RE-REVIEW-REQUIRED / RUNTIME-OFF` | تست‌های source، PostgreSQL و Redis، evidence exact-SHA و review مستقل successor باید کامل شوند؛ capability داخلی عمداً `False` است. |
| `4` | `BLOCKED-BY-STAGE-3-REVIEW / AUTHORIZATION-REQUIRED` | پس از re-review: preflight زنده `R25`، choreography `R12`، calibration `R28` و سپس workload نهایی staging. |
| `5` | `BLOCKED-BY-STAGE-4` | فقط پس از نتیجه Stage 4، تثبیت interval و مرور آمادگی production. |
| `6` | `NOT-AUTHORIZED` | production فقط با دستور صریح جداگانه؛ هیچ canary/deploy در این کار انجام نمی‌شود. |
...
- برای checkpoint هشتم cutover پیام‌ساز interaction Stage 3 در `2026-07-18`، سیزده outcome مستقل دعوت مشتری source key مجزا و `PRESERVE_CURRENT/NONE` گرفتند. promptهای cancel/confirm با `temporary_context_keyboard=true` ثبت می‌شوند، اما flow-exit مصنوعی یا persistent anchor جدید نمی‌سازند. پاسخ نهایی دعوت به‌دلیل edit بعدی پنل تا طراحی dependency/order حفظ شد. budget از `304` به `291` و total از `408` به `395` رسید؛ `22+573` تست متمرکز/گسترده سبز شدند.
...
- برای checkpoint سیزدهم cutover پیام‌ساز interaction Stage 3 در `2026-07-18`، entry منوی block با `PRESERVE_CURRENT/NONE` و source key مستقل به adapter مشترک منتقل شد؛ inline keyboard منو در payload canonical باقی ماند و caller هیچ نتیجه provider را مصرف نمی‌کند. دو callsite تکرار آفر با source بادوام موجود assertion نهایی legacy گرفتند و renderer بدون caller پنل حذف شد. سقف ماشینی inventory با checkpoint واقعی هم‌تراز شد: budget از `277` به `272`، total از `381` به `378` و `legacy_owner_guarded` از `10` به `12` رسید. تست race انقضای probe نیز با expiry کنترل‌شده پایدار شد؛ `15` تست Redis و `100/100` تکرار هدفمند، `37` تست متمرکز، `15/35/92` تست خانواده‌ای و `574` تست گسترده با `137` skip سبز شدند.
- برای checkpoint چهاردهم interaction Stage 3 در `2026-07-18`، قرارداد target pure به metadata بادوام known-message متصل شد و generic action outbox فقط `editMessageText` خصوصی authenticated را با target مثبت، هم‌مسیر و بدون ReplyKeyboard می‌پذیرد. replay با target متفاوت conflict است؛ handoff/freshness/feedback method و message id را end-to-end حفظ می‌کنند و `SENT_NOOP` ویرایش با target evidence به outbox `SENT` ختم می‌شود. adapter callback raw callback id را ذخیره نمی‌کند و تمام editهای منوی block را با source key outcome-specific صفی می‌کند. target نوع delivery-result عمداً تا طراحی persistence/dependency بعدی در ingress رد می‌شود. budget از `272` به `270`، total از `378` به `377` و `legacy_mode_guarded` از `66` به `67` رسید. `57+102+15` تست واحد/خانواده‌ای، `30+8` تست PostgreSQL bridge/anchor-edit، suite مرجع `585` و suite وسیع‌تر `677` تست با Redis واقعی سبز شدند؛ دو خطای stale mock Stage 5 که helper حذف‌شده قبل از این برش را patch می‌کردند نیز فقط در test fixture اصلاح شدند.
- برای checkpoint پانزدهم interaction Stage 3 در `2026-07-18`، تمام branchهای text-edit پیام همگانی مدیریتی target همان `callback.message.message_id` را با route authenticated و callback identity هش‌شده ثبت می‌کنند. source keyهای prompt/cancel/groups/selected/toggle/search/error/empty/queued از هم جدا هستند و replay یک outcome با outcome دیگر collision ندارد. سه caller مشترک prompt همان user احراز هویت‌شده را تا adapter حمل می‌کنند؛ موفقیت/خطای ساخت campaign نیز فقط پس از همان commit یا validation فعلی enqueue می‌شود. مرز markup-only با یک تست inventory صریح به‌عنوان تنها باقی‌مانده این خانواده قفل شد تا بعداً فقط با قرارداد واقعی `editMessageReplyMarkup` منتقل شود. budget از `270` به `261` و total از `377` به `368` رسید؛ `23+586+678` تست متمرکز/مرجع/وسیع‌تر با PostgreSQL و Redis واقعی سبز شدند و هیچ تستی skip نشد.
- برای checkpoint سخت‌سازی transport، retry و محرمانگی Stage 3 در `2026-07-19`، نتیجه gateway اکنون phase صریح `pre_write/write_unknown/response_received` دارد؛ response دریافت‌شده حتی اگر بستن client خطا دهد حفظ می‌شود و send با write نامعلوم هرگز کورکورانه retry نمی‌شود. شمار `provider_attempt_count` فقط در مرز dispatch بادوام افزایش می‌یابد و از admission/claim جداست. تنظیمات retry، lease، batch و limiter در startup ورودی non-finite، منفی، معکوس `base>max` و lease ناکافی را fail-closed رد می‌کنند؛ fallback backoff دارای jitter پایدار، محدود و مبتنی بر provider-attempt است، `retry_after` معتبر بدون jitter و فقط با safety رسمی اعمال می‌شود و overflow به بیشینه زمان قابل‌نمایش saturate می‌شود. migration افزایشی `a163f4a5b7c8` این دو fact را با constraint فیزیکی ثبت کرد و رفت‌وبرگشت `a163 → a052 → a163` روی PostgreSQL scratch پاس شد. log شکست gateway دیگر idempotency/dedupe خام را نمی‌نویسد و فقط correlation hash یک‌طرفه domain-separated دارد. اسکنر fail-closed جدید فایل، binary و آرشیو تو‌در‌توی ZIP/TAR/GZIP را برای token/credential و PII خام بررسی می‌کند، هیچ match حساسی را در خروجی تکرار نمی‌کند و گزارش JSON مناسب agent می‌سازد. شواهد نهایی `96` تست unit/worker/gateway/security و `84` تست PostgreSQL/Redis واقعی queue/resume/schema/retention، همراه `diff --check` سبز بود؛ هیچ شبکه provider، Telegram زنده، deploy، merge یا تغییر staging/production انجام نشد.
- برای checkpoint observability و محیط تست Stage 3 در `2026-07-19`، observer مستقل PostgreSQL فقط با transaction `READ ONLY` افزوده شد و health را به‌صورت aggregate و run-scoped برای depth/oldest-age، ingress/goodput/drain، retry/429، ambiguity/unresolved/blocked، lease منقضی، freshness missed، terminal failure، provider-outcome backlog و runtime gate می‌سازد. alertهای bounded دارای تصمیم ماشینی `continue/warning/stop` و exit code قطعی‌اند؛ metricهای dashboard فقط `priority`, `destination_class` و condition kind محدود دارند. shadow planner بدون row lock، lease، dispatch marker، gateway/http client یا DB mutation فقط readiness/order هر lane را با correlation hash یک‌طرفه گزارش می‌کند. runner الزاماً environment صریح `synthetic-test` یا `staging` و نام دیتابیس هم‌خوان می‌خواهد، `production` و نام production-like را بدون override رد می‌کند، provider network call را صفر ثبت و هر artifact را با scanner عمومی checkpoint قبل بررسی می‌کند. قرارداد اجرا و پاسخ alert در `docs/TELEGRAM_DELIVERY_QUEUE_OBSERVABILITY_RUNBOOK_20260719.md` و پیش‌نیاز Stage 4 ثبت شد؛ هیچ provider call، deploy یا mutation staging/production انجام نشد.
- برای checkpoint forward rollback در `2026-07-19`، downgrade کامل schema صراحتاً از راهبرد حذف شد: نقص قدیمی `5061c56d11e7` متعلق به این برنچ نیست و اجرای migration stage یک release قدیمی روی head جدید نیز امن نیست. checker جدید با transaction `READ ONLY` فقط وقتی `ready` می‌دهد که producer quiesce، backup manifest، skip migration، schema head یکتا، config دقیق legacy و شمار صفر active/leased/unresolved job، provider outcome معطل، resume ناتمام و runtime gate همگی اثبات شوند. terminal history روی schema افزایشی مجاز است و release legacy آن را نادیده می‌گیرد. مسیر شکست، roll-forward همان schema یا restore backup هم‌نسخه در دیتابیس جداست؛ downgrade/restore ناقص درجا ممنوع است. چهار تست PostgreSQL واقعی حالت ready، تمام blockerها، config/head mismatch و CLI read-only با artifact پاک را پوشش دادند و هشت تست unit/runtime policy نیز سبز شدند. rehearsal فیزیکی stop-old/drain/start-target همچنان در `R12` و پیش از production باقی است.
...
baselineهای `705` و سپس `368` فقط snapshotهای syntactic تاریخی بودند. ممیزی مستقل ثابت کرد guard در سطح کل تابع، alias و detached task می‌توانند false negative بسازند. ابزار جایگزین اکنون control-flow dominance را در scope صحیح می‌سنجد، import/callable alias، convenience method، nested scope، literal/dynamic `getattr` و task جداشده را پوشش می‌دهد و علاوه بر شمار دقیق، SHA-256 هویت callsiteها را pin می‌کند. `--check` روی هر callsite ناشناخته، هر `remaining_*` غیرصفر، تغییر شمار دقیق یا تغییر identity hash fail می‌شود. corpus بیست‌تایی adversarial نیز false guard، alternate root و bypassهای فوق را عمداً تزریق می‌کند.

| disposition | تعداد | نتیجه |
| --- | ---: | --- |
| `queue_execution` | `1` | gateway credential-bound رسمی صف |
| `legacy_owner_guarded` | `15` | assertion مالک legacy به‌صورت محلی و dominating پیش از مرز مستقیم قرار دارد |
| `legacy_mode_guarded` | `69` | شاخه queue در همان control-flow و پیش از فراخوانی مستقیم خارج می‌شود |
...
| `non_delivery_timer` | `5` | در queue mode فقط side effect غیرتلگرامی/legacy باقی می‌ماند و Telegram delivery نیست |
| تمام `remaining_*` | `0` | business، callback، interaction، cleanup و memory-delivery blocker صفرند |

جمع کل authoritative برابر `98` و callsite identity hash برابر `a7e5f6b27808625af362e7d091b6f7b2e8cae716ce730a7a5fd4cf7e790817bc` است. کاهش یا افزایش ظاهراً بی‌خطر هم بدون بازبینی inventory و به‌روزرسانی صریح hash مجاز نیست.

closure تاریخی پنج برش Stage 3 در snapshot پیش از review نهایی:

1. **انجام شد:** source/freshness restriction و حذف حساب، producerهای محلی/sync و OTP exemption محدود.
2. **انجام شد:** تمام callbackهای deadlineدار با ingress `M0` و زمان دریافت dispatcher.
3. **انجام شد:** تمام پیام/edit/documentهای interaction بر اساس نیاز واقعی به result، anchor، dependency و order؛ target وابسته به delivery از message id واقعی provider استفاده می‌کند.
4. **انجام شد:** cleanup و timerهای Telegram به scheduled operation بادوام منتقل یا در queue mode awaited شدند؛ پنج timer inventory‌شده فقط کار non-delivery/legacy دارند.
5. **انجام شد:** reconciler `AMBIGUOUS/PENDING_RECONCILE/blocked`، provider-outcome saga مستقل، retention و inventory معنایی بدون direct blocker.

این ادعای closure فقط وضعیت snapshot تاریخی را ثبت می‌کند و با checkpoint تاریخی و جدول authoritative ابتدای سند صریحاً supersede شده است؛ وضعیت جاری Stage 3 همان `REMEDIATED-LOCAL / RE-REVIEW-REQUIRED / RUNTIME-OFF` است. Stage 4 تنها پس از review successor SHA و مجوز جداگانه می‌تواند preflight یا تست زنده را آغاز کند.
...
| --- | --- | --- | --- | --- |
| `TOPQ-R01` | P0 | CLOSED-LOCAL | audit جایگزین dominance و scope را اثبات می‌کند، alias/convenience/dynamic-getattr/nested-scope/detached-task را می‌بیند و هم شمار دقیق و هم identity hash را pin می‌کند؛ هر `remaining_*` صفر است | `scripts/audit_telegram_delivery_calls.py`؛ `98` callsite، hash=`a7e5f6b...817bc` و `20` تست adversarial |
...
| `TOPQ-R07` | P0 | CLOSED-LOCAL | claim با `NOT EXISTS`های correlated، gate مقصد/resume/bot/gateway را پیش از lease کنار می‌گذارد و final serializable marker همچنان race را می‌بندد | PostgreSQL: M0 channel gated در برابر M1 private، hard-pause هر scope و final-boundary tests |
| `TOPQ-R08` | P0 | CLOSED-LOCAL | channel cooldown دیگر bot-wide استنباط نمی‌شود؛ primary پس از identity-only preflight فقط private را claim می‌کند و با پاک‌شدن gate برای full channel preflight به controller برمی‌گردد | unit lifecycle + PostgreSQL restart/resource tests؛ editor در gate کانال deferred می‌ماند |
| `TOPQ-R09` | P0 | CLOSED-LOCAL | 429 پیش‌پرواز پیش از sleep با deadline خام+safety و evidence هش‌شده در PostgreSQL ثبت، در claim enforce و پس از Redis loss rehydrate می‌شود؛ preflight موفق gate را auditably فعال می‌کند | `85144ed8`؛ PostgreSQL cooldown/crash/rehydrate و worker 429 tests |
| `TOPQ-R10` | P0 | CLOSED-LOCAL | pause bot/gateway و شناسه jobها transactionally پایدارند؛ resume دارای advisory fence، full preflight، journal DB/Redis، replay request-id، actor هش‌شده و CLI break-glass است | `85144ed8`؛ پنج PostgreSQL fault/race/restart، CLI و regression نه‌تایی resume مقصد |
| `TOPQ-R11` | P0 | CLOSED-LOCAL | fingerprint wire سازگار v4 از fingerprint implementation/NO_SYNC جدا شد؛ peer قدیمی فیلد افزوده را نادیده می‌گیرد و candidate fingerprint wire قبلی را حفظ می‌کند | `75f6ef4b`؛ mixed-version metadata compatibility test؛ choreography فیزیکی در `R12` |
...
| `TOPQ-R13` | P0 | CLOSED-LOCAL | مرز پذیرش transaction صریح شد؛ هر خطای Telegram/auxiliary پس از commit آفر پذیرفته‌شده را حفظ می‌کند و rejection یا copy غیرعادی به کاربر نشان نمی‌دهد | fault injection پس از commit و matrix هدفمند `88` تست در checkpoint بعدی `2026-07-18` |
| `TOPQ-R14` | P0 | CLOSED-LOCAL | حذف/غیرفعال‌سازی حساب در queue mode یک saga محلی و بادوام با دو job مستقل `banChatMember → unbanChatMember` می‌سازد؛ هر دو step binding و freshness منبع‌محور، lifecycle اتمیک، retry/429 غیرترمینال، state/error audit و permission readback صریح دارند. direct call فقط در legacy حفظ شده است. پس از موفقیت قطعی ban، unban یک compensation اجباری است و حتی با تغییر state یا relink بعدی supersede نمی‌شود تا kick موقت به ban دائمی تبدیل نشود | migration `ff41e2f3a5b6` با roundtrip واقعی؛ `112` تست unit/runtime/cutover، `32` تست bridge PostgreSQL، `78+1` تست queue/resume/schema و Redis واقعی؛ بدون provider زنده |
| `TOPQ-R15` | P1 | CLOSED-LOCAL | `idempotency_key` برای create API اجباری، محدود و validation‌شده است؛ replay/conflict و همه helperهای Web/API با قرارداد اجباری سازگار شدند | missing/blank/short/space/replay/republish tests در matrix هدفمند `88` تست |
| `TOPQ-R16` | P1 | CLOSED-LOCAL | policy صریح و exhaustive برای همه actionها ثبت شد؛ پیام‌های خصوصی حساس با تغییر route suppress می‌شوند و هر freshness adapter شناسه enqueue/audience را با مالک فعلی تطبیق می‌دهد | enum-coverage کامل و relink/privacy tests برای trade، broadcast، membership، repeat-offer و account notice |
| `TOPQ-R17` | P1 | CLOSED-LOCAL | admission، recent-429، probe/preflight و TTL set-max اتمیک از `Redis TIME` استفاده می‌کنند؛ `now` caller فقط validation/API metadata است و در gate مصرف نمی‌شود | `31` تست unit+Redis واقعی؛ ۵۰ contender با skew یک‌ساله و cooldown expiry واقعی |
| `TOPQ-R18` | P1 | CLOSED-LOCAL | `source_order_at` از `Offer.created_at` در job پایدار است و claim پس از priority/rank، catch-up/fresh/stale و newest-first را روی کل backlog اعمال می‌کند؛ fallback migration برای rowهای تاریخی rolling-safe است | migration `fe30d1e2f4b5`، roundtrip واقعی، depth میان feeder cycleها و کل `64` تست queue PostgreSQL |
| `TOPQ-R19` | P1 | CLOSED-LOCAL | retention زیر مالک queue به worker دوره‌ای foreign-only تبدیل شد: payload/response/error terminal پس از ۷ روز در batch قفل‌شده و idempotent redacted می‌شود؛ jobهای terminal پس از ۳۰ روز فقط در نبود legal hold، provider outcome حل‌نشده و source فعال purge می‌شوند. binding منبع terminal به‌صورت اتمیک جدا، زوج membership باهم حذف و dry-run واقعاً read-only است. metricها فقط label محدود phase دارند و هیچ ID/text/route حمل نمی‌کنند | migration `a052f3a4b6c7` و roundtrip واقعی؛ time-travel برای active/terminal/unresolved/legal-hold/dry-run/restart و membership pair؛ `4` تست PostgreSQL + schema/supervisor regression |
| `TOPQ-R20` | P0 | CLOSED-LOCAL | observer مستقل read-only گزارش run-scoped، metricهای bounded، alertهای backlog/age/ambiguity/blocked/freshness/provider-outcome، stop threshold و shadow بدون lock/mutation/provider call دارد | `telegram_delivery_observability_service`، CLI با exit code ماشینی، runbook و unit/PostgreSQL alert/read-only tests |
| `TOPQ-R21` | P1 | CLOSED-LOCAL | response معتبر پس از خطای close حفظ می‌شود؛ transport phase صریح `pre_write/write_unknown/response_received` در provider fact پایدار است و send با write نامعلوم ambiguous می‌ماند | migration `a163f4a5b7c8`؛ fault matrix gateway/contract و ماتریس کامل PostgreSQL provider-outcome |
| `TOPQ-R22` | P1 | CLOSED-LOCAL | startup ورودی non-finite/منفی، `base>max` و lease ناکافی را رد می‌کند؛ fallback retry jitter پایدار و محدود دارد، overflow saturate می‌شود و provider-attempt فقط در dispatch بادوام افزایش می‌یابد | config/contract fuzz-boundary و accounting واقعی claim/dispatch؛ `96` unit + `84` PostgreSQL/Redis |
| `TOPQ-R23` | P1 | CLOSED-LOCAL | gateway فقط correlation hash یک‌طرفه و domain-separated را log می‌کند؛ اسکن artifact فایل/binary/ZIP/TAR/GZIP برای secret و PII fail-closed است و مقدار match را echo نمی‌کند | gateway log-capture و پنج تست scanner شامل آرشیو تو‌در‌تو، CLI JSON و missing input |
| `TOPQ-R24` | P1 | CLOSED-LOCAL | راهبرد rollback به schema-preserving forward rollback یا restore مستقل محدود شد؛ checker read-only، legacy config، skip migration، backup و drain تمام stateهای صف را fail-closed می‌سنجد | `TELEGRAM_DELIVERY_QUEUE_FORWARD_ROLLBACK_RUNBOOK_20260719.md`؛ `4` PostgreSQL + `8` unit/runtime-policy؛ rehearsal فیزیکی در `R12` |
...
| `TOPQ-R26` | P1 | CLOSED-LOCAL | reconciliation با `origin/main@2c08da14` انجام شد؛ merge-base=`2d7c114d` و هر هفت commit جلوتر main در `git cherry` patch-equivalent (`-`) هستند، بنابراین merge اضافی لازم نیست | metadata bundle نهایی؛ هر تغییر بعدی `origin/main` این ردیف را خودکار باز می‌کند؛ بدون merge بدون تأیید |
| `TOPQ-R27` | P1 | CLOSED-LOCAL | runner فقط `synthetic-test|staging` را با نام DB هم‌خوان می‌پذیرد؛ production بدون override رد، transaction read-only و provider call صفر است | negative env/database tests، static no-gateway/http/row-lock assertion و artifact با environment صریح |
...
| `TOPQ-R29` | P0 | CLOSED-LOCAL | تمام producer/callsiteهای callback، interaction، pre-auth، cleanup و document در queue mode cutover شدند؛ result-dependent edit فقط message id واقعی provider را مصرف می‌کند و `sendDocument` بزرگ/مبهم fail-closed است | migration `a274f5a6b8c9`؛ audit صفر remaining، تست‌های pre-auth/document/anchor/dependency/retention و migration roundtrip PostgreSQL |
| `TOPQ-R30` | P0 | CLOSED-LOCAL | relink دیگر intent حساس قدیمی را به Telegram route جدید بازهدف‌گذاری نمی‌کند؛ immutable audience پیش از reclassification سنجیده می‌شود و provider fact قطعی از feedback دامنه مستقل و replayپذیر است | relink/privacy tests برای trade/admin/account/outbox؛ feedback-failure/restart با یک provider call |
| `TOPQ-R31` | P1 | CLOSED-LOCAL | runner محلی fail-closed فقط scratch localhost را می‌پذیرد، tokenها و queue runtime را خاموش می‌کند و ResourceWarning، unraisable، loop failure و رشد FD را شکست می‌دهد؛ raw log و JSON هم‌زمان ثبت می‌شوند و PostgreSQL session advisory lock مالکیت یکتای DB/Redis scratch را پیش از discovery تضمین می‌کند | `scripts/run_telegram_queue_local_evidence.py`؛ Telegram=`698` و Bot=`541`، هر دو zero-warning/zero-FD-growth؛ runner tests=`8` و rejection واقعی runner موازی |
| `TOPQ-R32` | P1 | CLOSED-LOCAL | evidence نهایی فقط از worktree تمیز و SHA ثابت ساخته می‌شود و شامل manifest، raw logs، JSON reports، main reconciliation، audit JSON و SHA256SUMS برای بازبین‌های محلی/GitHub است | artifact نهایی با نام مشتق از candidate SHA؛ scanner عمومی secret/PII باید پیش از تحویل PASS باشد |
...
1. `R01..R11`, `R13..R24`, `R26..R27` و `R29..R31` با کد و evidence محلی بسته شده‌اند.
2. worktree باید پس از کامیت نهایی تمیز باشد و bundle بند `R32` دقیقاً همان SHA را با scan و checksum ثبت کند؛ تغییر پس از آن review را باطل می‌کند.
3. `R12` در rehearsal مالکیت Stage 4، `R25` در permission readback زنده و `R28` در calibration واقعی همان کانال تعیین تکلیف می‌شوند. این‌ها open source task نیستند و هیچ‌کدام با mock قابل بسته‌شدن نیست.
4. تست نهایی `1800 valid + 400 invalid` فقط پس از preflightهای Stage 4 و با اجازهٔ اجرای staging آغاز می‌شود. production همچنان خارج scope و نیازمند دستور صریح جداست.

### 13.14 ثبت remediation بازبینی ChatGPT Pro روی `78df2e14` — `2026-07-19`

این جدول disposition هر finding را مستقل از شدت پیشنهادی reviewer ثبت می‌کند. `ACCEPTED` یعنی defect روی کد دوباره بازتولید یا از مسیر اجرایی اثبات شد؛ `HARDENING` یعنی ادعای شدت/قرارداد پذیرفته نشد ولی رفتار ورودی ایمن‌تر شد؛ `CONDITIONAL-LIVE` یعنی بدون اجرای مجاز staging قابل بسته‌شدن نیست.

| Finding | تصمیم | اصلاح یا دلیل رد | شاهد بسته‌شدن / گیت باقی‌مانده |
| --- | --- | --- | --- |
| `TQ-F001` | `P1 REJECTED / HARDENING ACCEPTED` | Bot API، `ResponseParameters.retry_after` را `Integer` تعریف می‌کند؛ بنابراین اعشار و عدد خارج از signed 32-bit پاسخ معتبر Telegram نیست و schema اعشاری لازم نیست. بااین‌حال contract و persistence اکنون دقیقاً یک parser سخت‌گیر مشترک دارند؛ bool/string/fraction/out-of-range بدون truncation وارد bounded fallback می‌شوند و 429 هرگز terminal نمی‌شود. | unit matrix malformed/fraction/string/bool/oversize؛ citation رسمی بخش ۱۵. |
| `TQ-F002` | `ACCEPTED / REMEDIATED-LOCAL` | eligibility، promotion، stale edit، lease creation/fence/recovery، provider replay، reconciliation و durable preflight/runtime gate/resume از `PostgreSQL clock_timestamp()` استفاده می‌کنند؛ deadline مطلق preflight Redis نیز از همان gate دیتابیس گرفته می‌شود. host time فقط metadata ورودی limiter است و Redis تصمیم admission را با `TIME` می‌گیرد. | تست واقعی host skew، lease fence، preflight cooldown و DB linearization ordering. |
| `TQ-F003` | `ACCEPTED WITH NARROWED GUARANTEE / REMEDIATED-LOCAL` | سه retry حذف شد؛ پاسخ قطعی تا commit نتیجه با backoff محدودشده ولی بدون سقف attempt در حافظه می‌ماند. barrier همان bot lane را از claim بعدی و همه lease recoveryها را تا ثبت fact متوقف می‌کند؛ cancellation پس از پاسخ تا commit defer و commit نامطمئن با fence یکتا retry می‌شود. crash کامل process/host قبل از commit همچنان صادقانه ambiguous است، زیرا Bot API idempotency token ندارد؛ ادعای تضمین پس از چنین crashی پذیرفته نشد. | outage بیش از چهار failure، commit failure، cancellation و barrier/recovery tests؛ replay بدون provider call دوم. |
| `TQ-F004` | `ACCEPTED / REMEDIATED-LOCAL` | hard permission block مقصد اکنون `(bot_identity,destination)` است؛ editor `403` فقط editor را می‌بندد. cadence و 429 cooldown واقعی همان channel همچنان بین botها مشترک‌اند و resume کامل هر دو hard key را فقط پس از saga/preflight پاک می‌کند. | PostgreSQL same-channel editor-403/primary-publication و Redis bot-scoped hard-block tests. |
| `TQ-F005` | `ACCEPTED / REMEDIATED-LOCAL` | primary دارای pool محدود قابل‌تنظیم با سه slot عمومی و یک slot رزروشده M0 است؛ editor slot مستقل خود را حفظ می‌کند. claim رزروشده effective priority—including trade promotion—را در SQL محدود می‌کند و limiter Redis authority نرخ/مقصد باقی می‌ماند. | slow lower-priority provider call + later M0 real PostgreSQL test؛ unit supervisor slot test؛ 300/100 gate Stage 4. |
| `TQ-F006` | `ACCEPTED / CLOSED-SOURCE-CONTRACT / LIVE-DRIVERS-CONDITIONAL` | generator قطعی ۶۰۰ثانیه‌ای با quota دقیق و causal-order validation، read-only redacted production-shape sampler، production-deny fingerprint guard، fault catalog، expected ledger، stop threshold، cleanup plan، sender/observer orchestration و verification fail-closed اضافه شد. verifier صرف وجود فایل یا `pass` خوداظهاری را نمی‌پذیرد و business/Telegram/receiver/fault/metric/cleanup/reconciliation/security ledgerها و SLOها را مستقل تطبیق می‌دهد. plan/live config به‌جز تغییر مجاز network-mode یک binding مشترک دارند و runner بدون flag و authorization exact-SHA هیچ process/provider را شروع نمی‌کند. | no-provider safety suite، ۲۵ seed causal-order، tamper/missing-receipt tests و dry-run checksum bundle؛ sender/observer واقعی، receiver receipt و cleanup فقط در Stage 4 مجاز بسته می‌شوند. |
| `TQ-F007` | `LOGICAL CONTRADICTION REJECTED / PRESENTATION FIXED` | سند از قبل یک جدول authoritative و عبارت صریح supersession داشت؛ بنابراین دو checkpoint تاریخی دو وضعیت هم‌زمان authoritative نبودند. بااین‌حال prose بالای سند برای جلوگیری از تفسیر اشتباه historical شد و یک test یکتایی جدول/status اضافه شد. | `test_telegram_queue_roadmap_consistency`; همین جدول تنها وضعیت جاری است. |
| `TQ-F008` | `ACCEPTED / REMEDIATED-LOCAL / CI-CONDITIONAL` | evidence runner اکنون test ID، outcome، duration، inventory hash، command redacted، Git SHA/dirty state، Python/OS/dependency version و raw verbose stream می‌نویسد. | exact-SHA local bundle؛ GitHub check مستقل در صورت وجود workflow/CI، وگرنه صریحاً `not_available` نه pass جعلی. |
| `TQ-F009` | `ACCEPTED AS SCOPE LIMIT / SPLIT GATE` | scanner اکنون exact tracked Git tree را با الگوهای high-confidence و artifact/archiveها را با الگوهای secret/PII کامل اسکن و blob/artifact scope را گزارش می‌کند. هیچ leak واقعی در review اثبات نشده بود. | source+evidence scan محلی؛ staging DB/container/central-log aggregate scan و scanner مستقل در Stage 4/production gate. |
| `TQ-F010` | `ACCEPTED / REMEDIATED-LOCAL` | report جدید برای هر commit main، counterpart کاندید و stable patch-id تولید می‌کند و raw `git cherry`، merge-base و divergence را ثبت می‌کند؛ absence mapping fail-closed است. | `report_telegram_queue_git_reconciliation.py` + synthetic differently-worded equivalent-patch test + exact refs artifact. |
| `TQ-F011` | `CONDITIONAL-LIVE` | نبود evidence زنده پیش از صدور مجوز defect source نیست و گزارش خود reviewer نیز آن را expected دانسته است. هیچ ادعای بسته‌شدن محلی جایگزین receiver evidence نمی‌شود. | Stage 4 Gates A–D/F و مجوز صریح مالک؛ production همچنان ممنوع. |
| `TQ-F012` | `CONDITIONAL-LIVE / NO-GO CAPABLE` | mismatch تحلیلی ظرفیت واقعی است، اما عدد امن با mock یا تغییر کد انتخاب نمی‌شود. یک کانال و semantics ثابت می‌ماند؛ editor ظرفیت chat را خودکار دوبرابر نمی‌کند. | حداقل سه calibration تکراری در هر mode/interval و معیارهای ADR-01؛ شکست برابر `NO-GO`. |

منابع کد این remediation شامل `telegram_delivery_queue_service`, `telegram_delivery_queue_worker`, `telegram_delivery_queue_limiter`, `telegram_queue_stage4_workload`, `telegram_queue_stage4_harness` و scriptهای sampler/reconciliation/evidence/scanner است. هیچ تغییر schema، merge، deploy یا provider call برای بستن بخش source-local انجام نمی‌شود.

### 13.15 ثبت remediation بازبینی مستقل `a1a94c1a` — `2026-07-19`

این جدول بخش `13.14` را برای هشت finding جدید supersede می‌کند؛ شناسه‌های قدیمی حذف یا بازنویسی نشده‌اند. بسته‌شدن محلی هر ردیف فقط پس از تست ذکرشده و evidence exact-SHA معتبر است و مجوز Stage 4 یا production ایجاد نمی‌کند.

| Finding | تصمیم نهایی | اصلاح الزام‌آور | شاهد بسته‌شدن |
| --- | --- | --- | --- |
| `TQ-R001` | `ACCEPTED / REMEDIATED-LOCAL` | preflight و delivery از parser واحد `Integer 1..2147483647` استفاده می‌کنند. bool/string/fraction/zero/negative/overflow/huge/missing در preflight نیز 429 باقی می‌مانند، به bounded fallback مثبت با hard cap برابر `300s` می‌روند و cooldown با منبع `bounded_malformed_fallback` در PostgreSQL ثبت می‌شود؛ Integer معتبر بدون cap حفظ می‌شود. | matrix کامل parser و preflight، caller-input یک‌تریلیون‌ثانیه‌ای؛ تست PostgreSQL cooldown malformed و max-Integer بدون انتظار واقعی. |
| `TQ-R002` | `ACCEPTED / REMEDIATED-LOCAL` | feeder publication/edit در هر transaction از `clock_timestamp()` نمونه می‌گیرد و همان timestamp را به candidate ordering، deadline و تمام handoffهای batch می‌دهد. fallback سرویس نیز مستقیماً PostgreSQL time می‌گیرد؛ ساعت host authority نیست. | unit propagation و real-PostgreSQL candidate selection با host skew منفی/مثبت ۲۴ ساعت؛ publication deadline یک ثانیه قبل/بعد DB time. |
| `TQ-R003` | `ACCEPTED / REMEDIATED-LOCAL` | گیت هم‌نقش در مرز نهایی provider بدون await خطی می‌شود؛ retained fact پیش و پس از claim و درست پیش از dispatch marker/provider بررسی می‌شود. dispatchهایی که قبل از بسته‌شدن واقعاً ثبت شده‌اند in-flight محسوب می‌شوند، اما slot صرفاً claimed/admitted دیگر provider call جدید آغاز نمی‌کند. هیچ mutex سراسری دور HTTP و serialization مصنوعی throughput ایجاد نشده است. | adversarial unit interleaving و تست دو slot واقعی PostgreSQL با retained provider fact؛ gateway slot دوم صفر call. |
| `TQ-R004` | `ACCEPTED / REMEDIATED-LOCAL` | plan اکنون `delivery_obligations.jsonl` کد‌مالک با source-version و causal dependency دارد: preview، confirmation edit، channel publication، editهای partial/معامله/انقضای دستی/انقضای خودکار، callbackها، پیام‌های دو طرف، broadcast و اعلان باز/بسته‌شدن بازار. callbackها نیز receipt مستقل لازم دارند. هر provider result schema دقیق و اتصال one-to-one دارد؛ `receiver_required` از driver پذیرفته نمی‌شود. latency، deadline ratio، reconciliation و acceptance از raw ledgers بازحساب و manifest اسکن از bytes artifactها دوباره ساخته می‌شود. | حذف مستقل publication/trade-party/terminal، duplicate effect/id، `receiver_required=false`، route/race/metric/reconciliation/cleanup/hash جعلی همگی fail؛ passing oracle verify می‌شود. |
| `TQ-R005` | `ACCEPTED / REMEDIATED-LOCAL` | scanner schema صریح `scanned_file_count + scanned_files[] + scanned_file_manifest_sha256` دارد؛ release report count کل را جدا نگه می‌دارد. verifier همان schema واقعی را می‌پذیرد، coverage همه artifactها را می‌سنجد و manifest را مستقل بازسازی می‌کند. | integration test خروجی واقعی `scan_release_surfaces` schema 3 را بدون دست‌نویسی به verifier می‌دهد و PASS می‌گیرد؛ hash ساختگی fail می‌شود. |
| `TQ-R006` | `ACCEPTED / REMEDIATED-LOCAL / INFRASTRUCTURE-ATTESTATION-REQUIRED` | runner دیگر `os.environ`، `PYTHONPATH` یا PATH والد را منتقل نمی‌کند؛ sender و observer allowlist و credential namespace جدا دارند. همه inputهای واقعی child با fingerprint امضاشده staging دوباره تطبیق می‌شوند؛ endpoint صرفاً نام‌گذاری‌شده staging کافی نیست. executable باید absolute/regular/non-symlink/executable باشد و digest هر دو driver در authorization قفل است. authorization Ed25519 کوتاه‌عمر، exact-field و یک‌بارمصرف است و کلید تأیید از trust-anchor مستقل، non-symlink و غیرقابل‌نوشتن برای group/other در خارج run directory می‌آید؛ config نمی‌تواند trust root خودش باشد. policy شبکه جدا قفل می‌شود؛ enforcement واقعی egress همچنان شاهد زیرساخت Stage 4 است. | signature/tamper/expiry/binary/extra-field، کلید self-declared مهاجم، permission/location فایل trust-anchor، environment-capture sender/observer، fingerprint-mismatch با production sentinel و preflight policy binding؛ egress enforcement گیت زنده است. |
| `TQ-R007` | `ACCEPTED / REMEDIATED-LOCAL` | هر ۱۸ race دقیقاً یک expiry و حداقل دو trade روی barrier مشترک دارد؛ planned skew حداکثر ۲۵ms و ۹ winner معامله/۹ winner انقضاست. verifier observed sender skew حداکثر ۱۰۰ms و دقیقاً یک outcome authoritative را الزام می‌کند. همه مجموعه‌های generator به ترتیب canonical پیمایش می‌شوند تا trace مستقل از `PYTHONHASHSEED` باشد. | ۲۵ seed با ۱۸ گروه واقعی و planned skew≤50ms؛ mutation outcome/skew fail؛ hash بین‌پردازشی یکسان. |
| `TQ-R008` | `ACCEPTED / REMEDIATED-LOCAL` | fault catalog شکل‌های `1`, signed-32 max، missing، bool، numeric string، fractional، zero، negative، max+1 و huge integer را جداگانه با shape hash، expected source و allowlist state بادوام ثبت می‌کند. verifier provider-call count، duplicate effect، state مجاز، applied seconds و deadline source را الزام می‌کند و `status=pass` یا state دلخواه authority نیست. | fault-catalog equality، parser matrices، malformed/max persistence tests و mutation state جعلی در live evidence. |
...
### 13.16 ثبت remediation بازبینی مستقل `ef15604d` — `2026-07-19`

این بخش وضعیت authoritative پنج finding `EF-RV-001..005` است و ادعای closure بخش `13.15` را supersede می‌کند. `REMEDIATED-LOCAL` فقط به معنی وجود اصلاح و تست source است؛ تا کامیت تمیز، اجرای ماتریس exact-SHA، اسکن، push و re-review مستقل، هیچ finding بسته نهایی و هیچ اجرای زنده مجاز نیست.

| Finding | تصمیم | اصلاح الزام‌آور انجام‌شده | شاهد بسته‌شدن و rollback امن |
| --- | --- | --- | --- |
| `EF-RV-001` | `ACCEPTED / REMEDIATED-LOCAL` | کل supervisor شامل primary و editor پیش از ساخت هر lane یک PostgreSQL session advisory lock یکتا می‌گیرد. connection و backend PID تا پایان عمر worker نگه داشته می‌شوند؛ monitor و مرز بلافاصله پیش از provider هم PID و هم وجود lock در `pg_locks` را می‌سنجند. اجرای cycle بدون lease، lease گم‌شده یا session تعویض‌شده job را پیش از dispatch آزاد و provider را صفر-call می‌کند. | unit acquire/reject/session/lock-loss؛ تست دو Python process روی PostgreSQL مشترک که مالک دوم را تا release رد می‌کند؛ تست real-session `assert_held`. rollback: runtime flag خاموش و فقط یک owner قدیمی؛ هیچ migration جدیدی ندارد. |
| `EF-RV-002` | `ACCEPTED / REMEDIATED-LOCAL` | ۱۶۲ انقضای دستی عادی فقط از آفرهای غیرمعامله و ۱۸ overlap فقط از raceهای صریح انتخاب می‌شوند. برای هر آفر بدون terminal معامله/دستی، رویداد automatic expiry دقیقاً در `submit+120s` تا انتهای drain ثبت می‌شود. market eventها notification-only نام‌گذاری شده‌اند. هر ۱۰۸ partial trade فقط روی آفر ۲ یا ۳ لات است. | validator دقیق overlap/automatic-expiry/terminal coverage، آزمون ۱۰۰ seed و hash-seed مستقل؛ regression همه quotaها. rollback: generator provider-free است و شکست validation پیش از هر process/network متوقف می‌شود. |
| `EF-RV-003` | `ACCEPTED / REMEDIATED-LOCAL` | terminal edit دیگر هنگام confirmation ساخته نمی‌شود؛ obligationهای پیام دو طرف، partial و terminal فقط با نتیجه committed واقعی فعال می‌شوند. هر ۱۶۲ گروه concurrent دقیقاً یک winner دارد و loserها پاسخ عادی خود را حفظ می‌کنند. observer snapshot خام status/remaining lots/version/terminal event را می‌دهد و verifier hash را بازتولید می‌کند. provider result به state digest، payload digest و fingerprint واقعی message target وصل است؛ receiver مستقل payload/state/buttons را برای `sent` و `sent_noop` بازخوانی می‌کند. partial edit پس از terminal فقط با outcome `superseded` معتبر است. | passing oracle کامل `1800+400`؛ negativeهای terminal-before-commit، wrong text/buttons با امضای معتبر، state ساختگی با امضای معتبر، zero/multiple winner، no-op بدون receipt و partial-after-completion. rollback: obligation ناسازگار fail-closed و provider execution مجاز نمی‌شود. |
| `EF-RV-004` | `ACCEPTED / REMEDIATED-LOCAL` | sender و observer با UID/GID غیرroot و متفاوت، supplemental group خالی و output directoryهای جداگانه `0700` اجرا می‌شوند؛ هر role فقط فایل‌های allowlist خود را با Ed25519 attestation مستقل تحویل می‌دهد. fault matrix به sender-signed raw injection/execution و observer-signed raw durable observation شکسته شد. runner فقط inventory تخت دقیق را promote می‌کند، scanner را خودش روی تمام bytes اجرا می‌کند و هر extra/directory/symlink یا clean report دست‌ساز را رد می‌کند. | adversarial write واقعی دو UID در هر دو جهت، signature/file mutation، exact inventory، symlink، scanner recomputation، raw-state/hash forgery و fault provenance tests. rollback: directoryهای موقت حذف و authorization مصرف‌شده باقی می‌ماند؛ retry نیازمند authorization تازه است. |
| `EF-RV-005` | `ACCEPTED / REMEDIATED-LOCAL` | CLI هیچ trust path/key قابل‌انتخاب ندارد و فقط policy ثابت root-owned زیر `/etc` را می‌خواند. policy به machine identity، سه public key متمایز، دو private/public pair واقعی، UID/GIDها، registry ثابت و driver root قفل است. authorization nonceدار و حداکثر یک‌ساعته در registry خارجی به‌صورت اتمیک یک‌بار مصرف می‌شود. driver فقط یک executable root-owned، بدون write bit و زیر root ثابت است و digest پیش از launch دوباره کنترل می‌شود. ready receipt مالک observer، mode `0600`، تازه، PID-bound، executable-bound و Ed25519-signed است. planهای root-owned پیش و پس از childها verify می‌شوند. | forged caller key/nonce، replay در directory کپی، policy host mismatch، key-pair mismatch، mutable driver، unsigned/stale/wrong-permission ready و authorization evidence chronology/hash tests. rollback: پیش از sender شکست می‌خورد؛ authorization استفاده‌شده قابل‌بازیابی نیست و approval تازه لازم است. |

مواردی که این remediation عمداً «بسته زنده» اعلام نمی‌کند: enforcement واقعی egress، permission readback بات‌ها/کانال، ظرفیت واقعی همان channel، smoke واقعی message ID/payload، اجرای workload نهایی، cleanup زنده، rollback rehearsal و CI محافظت‌شده. این موارد به‌ترتیب gateهای Stage 4/5 هستند، نه مجوز تغییر SLO یا دورزدن safety. در activation child، مقدار `TELEGRAM_DELIVERY_QUEUE_IMPLEMENTATION_READY=True` فقط وجود قابلیت بررسی‌شده در کد را بیان می‌کند؛ فعال‌شدن runtime همچنان به کنترل‌های مستقل owner/worker/cutover و مجوز صریح نیاز دارد.
