...
- وضعیت Roadmap: Stage 0 و Stage 1 ثبت شده‌اند. ممیزی closure در `2026-07-16` تمام تصمیم‌های اولیه challenge register را بست؛ شش ADR اولیه و قرارداد pure نهایی `M0` تا `M7` ابتدا با `44` تست قرارداد و `171` تست هدفمند/regression پاس شدند. همان روز ADR هفتم برای `channel_editor` پذیرفته و برش foundation مرحله ۳ ممیزی شد: پس از merge آخرین `main`، `316` تست unit/regression و `20` تست PostgreSQL واقعی، همراه upgrade→downgrade→upgrade migration پاس شدند. طراحی scheduler برای حذف head-of-line blocking بین ظرفیت primary و editor اصلاح و foundation دو execution lane مستقل و work-conserving آن پیاده شد؛ credential registry بدون fallback، limiter پایدار Redis، preflight هویت/کانال/permission، canonical publisher identity، freshness authoritative خانواده publication/edit آفر و اعلان بازار، و handoff/feedback اتمیک نتیجه خصوصی معامله نیز پیاده شده‌اند. ownership و state machine همچنان یکی است. runtime صف هنوز code-disabled و Stage 3 کامل نیست؛ هیچ deploy انجام نشده و فعال‌شدن editor منوط به تکمیل actionها و feederهای باقی‌مانده، reconciliation عملیاتی، اجرای readback زنده و smoke/benchmark Stage 4 است.
...
- `invalid_action_button_edit` در mapping اولویت قرارداد رزرو شده اما هنوز producer یا رکورد authoritative ندارد؛ producer/handoff واقعی و route عمومی بر پایه payload صف ممنوع است و این شکاف عمداً activation هر دو lane را fail-closed نگه می‌دارد. freshness سایر actionها شامل callback، success خصوصی آفر، پاسخ/پیشنهاد جایگزین/عدم‌دسترسی درخواست معامله، پیام مدیریتی و cleanup زمان‌دار، به‌همراه تکمیل coverage router و lifecycle-feedback مرکب هر lane، feeder cutoverهای باقی‌مانده و reconcile پایدار jobهای `AMBIGUOUS`/`PENDING_RECONCILE`/blocked باید در ادامه Stage 3 تکمیل شوند. مسیر عملیاتی resume نیز باید DB state، preflight کنترل‌شده و Redis را یکپارچه و auditپذیر هماهنگ کند؛ `resume_*` صرفاً Redis مجوز فعال‌سازی نیست. OTP خارج shared queue باقی می‌ماند.
