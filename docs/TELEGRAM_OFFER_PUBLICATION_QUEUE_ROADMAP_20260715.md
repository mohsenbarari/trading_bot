...
- `invalid_action_button_edit` در mapping اولویت قرارداد رزرو شده اما هنوز producer یا رکورد authoritative ندارد؛ producer/handoff واقعی و route عمومی بر پایه payload صف ممنوع است و این شکاف عمداً activation هر دو lane را fail-closed نگه می‌دارد. freshness سایر actionها شامل callback، success خصوصی آفر، پاسخ/پیشنهاد جایگزین/عدم‌دسترسی درخواست معامله، پیام‌های مدیریتی هدفمند غیر-broadcast مانند عضویت/حساب، و cleanup زمان‌دار، به‌همراه تکمیل coverage router و lifecycle-feedback مرکب هر lane، feeder cutoverهای باقی‌مانده و reconcile پایدار jobهای `AMBIGUOUS`/`PENDING_RECONCILE`/blocked باید در ادامه Stage 3 تکمیل شوند. broadcast انبوه مدیریتی در برش `2026-07-18` به صف اصلی متصل شد. مسیر عملیاتی resume نیز در همان تاریخ بسته شد؛ `resume_*` صرفاً Redis همچنان مجوز فعال‌سازی نیست. OTP خارج shared queue باقی می‌ماند.
