...
مواردی که این remediation عمداً «بسته زنده» اعلام نمی‌کند: enforcement واقعی egress، permission readback بات‌ها/کانال، ظرفیت واقعی همان channel، smoke واقعی message ID/payload، اجرای workload نهایی، cleanup زنده، rollback rehearsal و CI محافظت‌شده. این موارد به‌ترتیب gateهای Stage 4/5 هستند، نه مجوز تغییر SLO یا دورزدن safety. runtime همچنان باید `TELEGRAM_DELIVERY_QUEUE_IMPLEMENTATION_READY=False` بماند.
