...
خروجی baseline بازبینی‌شده `705` مرز syntactic را ثبت می‌کند. این ممیزی محافظه‌کارانه است: reachable بودن هر مسیر در یک deployment را ادعا نمی‌کند، اما هیچ فراخوانی را فقط به‌دلیل «احتمالاً بلااستفاده بودن» از inventory حذف نمی‌کند.
...
| `legacy_mode_guarded` | `57` | شاخه queue پیش از فراخوانی مستقیم خارج می‌شود |
| `legacy_parameter_guarded` | `2` | caller در queue mode شاخه Telegram را صریحاً خاموش می‌کند |
| `durable_exempt` | `2` | فقط مسیر OTP کوتاه‌عمر مصوب |
...
| `remaining_business_direct` | `6` | blocker Stage 3 |
| `remaining_callback_direct` | `292` | blocker Stage 3 |
| `remaining_interactive_direct` | `312` | blocker Stage 3 |
| `remaining_cleanup_direct` | `13` | blocker Stage 3 |
| `remaining_memory_timer` | `7` | blocker Stage 3 |

شش مرز business باقی‌مانده چهار مسئله منطقی را نشان می‌دهند: اعلان اعمال block/limitation، اعلان حذف حساب در مسیر محلی و sync، relay قدیمی notification/OTP و wrapper مستقیم آن. برای رفع آن‌ها استفاده کور از قرارداد عمومی `user_route` ممنوع است؛ پیام restriction باید snapshot همان restriction را revalidate کند و اعلان حذف باید پس از پاک‌شدن `telegram_id` با route snapshot مجاز و state حذف authoritative تطبیق داده شود. relay قدیمی نیز باید به OTP محدود و در مرز دریافت fail-closed شود، نه اینکه یک تونل عمومی پیام خارج صف باقی بماند.

ترتیب قطعی برش‌های بعدی Stage 3:

1. قراردادهای source/freshness اعمال restriction و حذف حساب، سپس cutover هر چهار producer محلی/sync و محدودکردن relay قدیمی به OTP.
2. تبدیل callbackها به‌صورت خانواده‌ای با ingress مستقیم `M0` و deadline دریافت‌شده؛ هیچ callback نباید منتظر feeder polling بماند.
3. تعریف قرارداد نتیجه/anchor برای interactionهایی که در رفتار فعلی به `Message.message_id` برگشتی وابسته‌اند، سپس cutover پیام‌های خصوصی بر اساس صف تابع مصوب. تبدیل مکانیکی `message.answer` بدون این قرارداد ممنوع است.
