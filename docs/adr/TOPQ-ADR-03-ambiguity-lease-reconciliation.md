...
## الحاقیه persistence و clock پس از review نهایی `2026-07-19`

- زمان همه transitionهای execution—claim، promotion، lease، dispatch fence، result، retry، provider replay، recovery و reconciliation—از `PostgreSQL clock_timestamp()` می‌آید. timestamp صریح فقط seam تست deterministic است؛ process wall clock authority پایدار نیست. limiter Redis همچنان فقط از `Redis TIME` تصمیم می‌گیرد.
- پس از دریافت پاسخ قطعی provider، worker آن fact را تا commit موفق با retry بدون سقف attempt در حافظه نگه می‌دارد. در این بازه همان bot lane claim جدید نمی‌کند و lease recovery سراسری اجرا نمی‌شود؛ cancellation ثبت‌شده فقط پس از durable شدن fact اعمال می‌شود. commit نامطمئن با unique fence همان `(job_id,lease_token)` دوباره تلاش می‌شود.
- این fail-stop تضمین را فقط تا وقتی process زنده است می‌بندد. crash کامل host/process میان پاسخ Telegram و اولین commit همچنان `AMBIGUOUS` است؛ local spool حاوی response ممکن است privacy/secret durability جدیدی بسازد و بدون ADR و threat model جدا اضافه نمی‌شود.
