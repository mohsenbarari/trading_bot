...
- مقدار `50` سقف ایمنی staging است، نه هدف رفتار کاربر؛ در بار ترکیبی اصلی soft guard هر owner برابر `10` آفر فعال است و عبور از آن anomaly سناریو محسوب می‌شود.
- generator باید آفرها را طوری توزیع کند که سقف ۵۰ هیچ‌وقت عامل مصنوعی رد آفر نباشد و در عین حال بار روی چند chat خصوصی متمرکز نشود.
