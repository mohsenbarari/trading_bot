...
### 10.6 گیت نبود head-of-line blocking میان دو lane

این گیت بعد از cross-bot smoke و پیش از benchmark بار ترکیبی اجرا می‌شود:

1. حداقل `300` job آماده `M0/M1` با `bot_identity=primary` در backlog پایدار نگه داشته می‌شود؛ دست‌کم `200` مورد private/admin و خارج gate کانال‌اند و باقی ترکیب شامل callback/trade/publication است تا scheduler فقط روی یک method یا مقصد بهینه نشده باشد.
2. هم‌زمان `100` edit واجد ارسال با `bot_identity=channel_editor` روی message idهای canonical وارد می‌شود.
3. تا وقتی backlog primary هنوز بزرگ‌تر از صفر است، lane editor باید claim و dispatch موفق ثبت کند. priority، sleep، retry، semaphore یا in-flight primary دلیل مجاز انتظار editor نیست.
4. در زیرآزمون non-channel، backlog private/admin primary نمی‌تواند دلیل توقف editor باشد. هر بازه توقف editor فقط وقتی پذیرفته است که ledger gate مقصد خود editor یا circuit مشترک فعال را با زمان شروع/پایان و علت provider/config نشان دهد.
5. زیرآزمون channel-only با backlog publication primary تکرار می‌شود تا انتظار مشروع gate مشترک مقصد از cross-lane head-of-line نرم‌افزاری جدا شود؛ انتظار بدون شاهد gate شکست است.
6. تست معکوس با backlog edit تکرار می‌شود و باید ثابت کند publication/private primary پشت lane editor متوقف نمی‌شود.
7. شمار claim با bot identity اشتباه، fallback، duplicate و route mutation در هر دو جهت باید صفر باشد.

این سناریو افزایش ظرفیت همان کانال را فرض نمی‌کند؛ فقط ثابت می‌کند scheduler ظرفیت دو credential را با یک dispatcher سراسری مصنوعی از بین نمی‌برد.
...
### A4. جداسازی ظرفیت دو lane

- اجرای کامل گیت بخش ۱۰.۶ با `300` job پر‌اولویت primary، شامل حداقل `200` مقصد private/admin، و `100` edit editor
- تکرار جهت معکوس برای اثبات اینکه backlog editor نیز primary را نگه نمی‌دارد
- تفکیک دقیق زمان انتظار `bot_lane`، `destination_gate` و `circuit`
- شکست این اجرا mode editor را پیش از load ده‌دقیقه‌ای رد می‌کند
...
- اشباع هر lane، توقف/restart مستقل runner آن و اثبات ادامه lane دیگر مگر هنگام circuit یا destination gate دارای شاهد
...
- در سناریوی `300` job آماده `M0/M1` primary که حداقل `200` مورد آن private/admin است، همراه `100` edit editor، editor باید پیش از صفرشدن backlog primary پیشروی کند؛ انتظار `cross_lane_priority` یا `cross_lane_worker` برابر صفر و هر توقف دارای شاهد gate مقصد editor/circuit باشد. زیرآزمون channel-only انتظار gate مشترک را جدا گزارش کند و تست معکوس نیز همین جداسازی را برای primary اثبات کند.
...
- cooldown و backlog gate مشترک channel destination در کنار budget و execution lane جداگانه هر bot role
- claim rate، in-flight، utilization و oldest-ready-age به تفکیک execution lane
- زمان انتظار به تفکیک `bot_lane`, `destination_gate`, `circuit` و شمار `cross_lane_head_of_line`
