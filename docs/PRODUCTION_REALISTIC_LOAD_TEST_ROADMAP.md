...
Status: Stage L8 is closed as a spike characterization. Stage L7 proved the current official target at `500 RPS / 10m`. Stage L8 then pushed the same accepted diagnostic profile to `750 RPS / 5m`: the system degraded gracefully with `0%` request failures and `0` new Nginx 5xx delta, but latency gates crossed at p95 `1957.7ms` and p99 `3453.6ms`. Treat `500 RPS` as the current release capacity target and treat `750 RPS` as above-target saturation evidence, not accepted release capacity.
...
| `L9`-`L11` | Pending | soak, final analysis, and release-capacity decision remain pending after the L8 spike closure. |
