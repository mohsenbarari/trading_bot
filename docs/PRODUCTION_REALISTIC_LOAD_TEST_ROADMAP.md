...
Status: Stage L7 is closed. The first official target attempt exposed DB pool exhaustion, API worker saturation, and Nginx upstream resets; L7.1-L7.8 isolated and fixed those blockers. The final official `500 RPS / 10m` run passed on 2026-06-13 with `LOAD_RUNNER_SHARDS=2`, `workers=24 pool=10:4`, `0%` request failures, p95 `559.76ms`, p99 `812.07ms`, effective throughput `498.21 req/s`, dropped-iteration ratio `0.324%`, max PostgreSQL connections `248`, `0` new Nginx 5xx delta, and production env restored to baseline.
...
| `L8`-`L11` | Pending | spike, soak, analysis, and release-capacity decision remain pending after the L7 target closure. |
