...
چرخهٔ زنده فقط projection افزایشی را اجرا می‌کند و `quick_check` کامل SQLite
را تکرار نمی‌کند؛ بررسی کامل هر سه پایگاه در پشتیبان/restore-smoke و گیت نصب
انجام می‌شود. اسکن کامل در هر چرخه با رشد Market Store، تازگی ورودی مدل را از
بین می‌برد و کنترل صحت را به عامل اختلال تبدیل می‌کند.
