...
- implemented: WebApp-request duplicate idempotency replay stress paths for
  user-to-user stable scenarios across WebApp and Telegram offer origins, both
  offer types, and all current offer shapes;
...
- current whole-manifest command-plannable count: `86` of `5555` scenarios;
...
2. `specialized_user_stress_driver`: `84` gaps. Extend the dual-role worker for
   Telegram duplicate replay, expiry races, and read-during-write.
