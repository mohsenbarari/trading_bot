...
- وجود `توضیحات:` به‌تنهایی شرط نیست. یادداشت عادی در fact store و مسیر
قیمت عادی می‌ماند، بدون ذخیره‌شدن متن خام.
- شرط روشن (فیش یک‌فقره، مهلت پرداخت، اجبار پرداخت، تعویض یا شرط صریح) و
یادداشت پرداختی مبهم با `is_conditional=true` و دلیل/کلاس privacy-minimized
ثبت می‌شود. این رویدادها حذف نمی‌شوند.
- رویداد شرطیِ فیزیکال فقط اگر با دست‌کم دو رویداد غیرشرطیِ پیشین در همان
کتاب فیزیکال و حداکثر ده دقیقهٔ قبل قابل‌مقایسه باشد، وارد Snapshot و
موتور نرخ می‌شود. آستانهٔ انحراف از median مرجع تطبیقی اما بین `۰.۴۵٪` تا
`۱.۲۵٪` محدود است؛ بنابراین شرط ویژه یا قیمت دور از بازار روی تخمین لحظه‌ای
اثر ندارد، اما fact آن برای تحلیل و آموزش آینده محفوظ می‌ماند.

## factهای منفرد و minute quote

فیزیکال‌ها aggregate نمی‌شوند: هر offer و trade مستقل ثبت می‌شود. یادداشت
عادی مانند رویداد معمولی استفاده می‌شود؛ شرط فقط پس از gate بالا وارد نرخ
لحظه‌ای می‌شود.
