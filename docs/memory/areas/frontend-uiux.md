...
- 2026-08-12 | Workspace relation mutations accept only matching `revoked`/`deleted` receipts; malformed/network failures retain context with fixed safe copy. Reason: prevent raw-detail leaks and stale reconciliation.
- 2026-08-12 | Workspace account deletion uses a body-teleported V2 portal, exact name/acknowledgement, locked `expected_action=delete-account`, receipt checking and fixed safe failure copy. Reason: prevent unsafe action from raw details, stale receipts or clipped overlays.
