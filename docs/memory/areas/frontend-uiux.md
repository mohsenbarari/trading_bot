...
- 2026-08-15 | Market owner correction supersedes the SVG perimeter: an inset linear meter has no side rail or reset transition; normal time depletes while overtime jumps to zero then fills from `final_deadline`. Expired/untraded, partial and full trades are gray/amber/teal read-only states. Web trade payloads normalize positive integers, include `offer_public_id`, retain idempotency, and explain structured 422 fields. The legacy cross-channel warning is not rendered. Protected semantics remain guard-bound.
