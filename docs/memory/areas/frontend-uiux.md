...
- 2026-08-15 | Market owner corrections: the inset linear meter has no side rail, reset animation, or visible countdown; normal depletes and overtime jumps to zero then fills. Expired, partial, and full trades stay gray/amber/teal. Web two-tap keeps first confirmation inside the 44px action button and cards compact; no recap panel/cross-channel warning. Trade payloads normalize IDs/quantity, include public ID, retain idempotency, and explain 422 fields. Protected semantics are guard-bound.
