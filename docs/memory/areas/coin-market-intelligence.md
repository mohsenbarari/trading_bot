...
- Units: project-thousand; accept full-Toman/separators/zeros. Resolve tails by family or unique causal near-time match, never constants. `رب`=quarter; `پ/ت پ/پایین`=low-date. Duplicated-zero low-price needs a family.
...
- Estimator: `estimator-live`, facts-only `/shadow`, events=120s; no-data=0/failure=3. Inputs: private gold/Herat/XAU/G1/G2. Weak evidence abstains; prefer private melted.
...
- Private melted: price/quantity immutable; lifetime=120s. Lower remaining=cumulative fill; zero=full, positive closure=no-trade. Generic edit≠trade; inconsistency=ambiguous; partials finalize at deadline; freshness=900s.
...
- 2026-08-29/30: Shadow→Legacy feeds G1/G2/private-gold; Product stays `LEGACY`. Exact audit since `2026-08-25T09:33Z`: offers 1,249 (1,231/18; SHA `0ef168f6…df0a`), trades 277 (210/67). Reviews are causal, learn no syntax without raw, and use `human-feedback-r*`.
