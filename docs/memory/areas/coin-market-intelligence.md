...
- USD/Herat prefix: 3 facts/book-source/15m; no constants/future replay.
- خ ن ف/ف ن ف، خ ف/ف ف یا none: tomorrow؛ خ ن/ف ن/نق: cash. Future wins.
- Prices: project-thousand; full-Toman /1,000; bounded zeros. Inputs 1–3 digits ×1,000؛ 4 invalid. Reject quantities/scripts/years. `رب`=quarter؛ `پ`/`ت پ`/`پایین`=low-date.
...
- Private text: authenticated review only; opaque projection. Live jobs: `main`.
...
- Staging catalog mirrors Iran؛ Snapshot read-only. v3 exact bands; same-family ±10% confirmation; ties require choice. Edits reuse scope, never auto-select; bot rejects stale taps.
...
- Collectors never go in dashboard/bridge `After=`؛ use `flock`.
