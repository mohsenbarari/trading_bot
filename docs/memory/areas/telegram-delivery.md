...
- 2026-09-05 | Staging split has `primary` + `executor` but one Queue owner; redeploy quiesce/recovery/evidence covers both.
- 2026-09-03 | Redeploy adopts the `AUTHORITY_TRANSFERRED` Market lock after quiescence and retains its inode.
- 2026-09-03 | Expiry/final-tail preserve Telegram posts/buttons; overtime adds `⏳`. Stale clicks fail authoritative validation; trade edits remain unchanged.
...
- 2026-08-23 | Panel precedes FSM/router; admin uses configured origin/dev key. Callbacks ACK first; stale actions fail visibly. Latency uses ACK wakeup, shared HTTP, batch 8, `sent` index and 1.05s/destination; local ACK is fenced.
...
- 2026-08-19 | OTP uses encrypted Redis, ACK+DELETE/quarantine; PostgreSQL only wakes. Tables decide, success follows commit, retired Mini App stays retired.
