...
- 2026-08-17 | Staging Telegram execution is Queue-v1 on the bot host; API/WebApp stay producer-only. Login OTP is an encrypted foreign Redis stream executed only by the bot. Terminal OTP commands must ACK+DELETE; health reports real outstanding (group lag + pending), never historical XLEN; max-deliveries comes from Redis `times_delivered`. SMS is Iran-only and may stay BLOCKED without blocking Telegram OTP. A 100-offer matrix revalidates these fixes; it does not replace the historical 500-offer rollout. Recipient-less residue stops health and uses the audited reconciler; never use direct SQL/provider cleanup. Production remains Legacy pending separate owner authorization.
