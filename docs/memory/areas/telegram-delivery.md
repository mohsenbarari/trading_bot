...
- 2026-08-17 | Staging Telegram execution is Queue-v1 on the bot host; API/WebApp stay producer-only. Login OTP is an encrypted bot-owned Redis stream: terminal commands ACK+DELETE, retry metadata uses exact stream IDs, and poison quarantine is idempotent. Queue payloads may retain `None` for hashing/freshness, but the gateway omits those optional fields at the Telegram HTTP boundary. Publisher executors must cover their offer actions plus both owner-bound callback actions; other private/admin work stays forbidden. Health reports outstanding work, never historical XLEN. SMS may stay BLOCKED without disabling Telegram OTP. Use only the audited residue reconciler. Production remains Legacy pending separate owner authorization.
