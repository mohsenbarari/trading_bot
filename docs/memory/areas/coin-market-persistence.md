...
- Market Facts use an mTLS/HMAC lane independent of product sync. Runtime is Docker-native: commit-bound image, isolated commands/volumes, no baked state/secrets/network SQLite; locks prevent concurrent owners. Legacy stays for parity/rollback.
...
- Stages 8–10 separate logical/delivery sequence with atomic outbox, contiguous ACK, bounded repair, guarded mapping and explicit feed lanes. Snapshots carry durable ACK/web apply and timing/input trace; route loss becomes stale and mapping/storage/sequence fail closed.
...
- Stage 12 parity is redacted/signed: one capture owner, exact receipt window/shared clock, cloned store/aligned snapshots and no raw scratch. Separate value/metadata/schema drift; minute XAU is not an event oracle. Parser drift needs labels; replay cannot replace the open-market gate.
...
- Adapter backlog reads are bounded to 500/cycle with per-stream cursors and causal heap merge; unbounded payload fetch/sort is forbidden.
