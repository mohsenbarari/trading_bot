...
- Facts: mTLS/HMAC, no PII, PostgreSQL Decimal/UTC and outbox authority. Facts plus identity/revision lineage are permanent: 180d hot then compressed cold; sources include G1/G2, melted, Herat, USDT and future Iranian exchanges.
...
- Parity freezes owner prefix; replay never replaces live gates.
...
- 2026-09-05: With capture `AUTHORITY_TRANSFERRED`, direct legacy Telegram collectors stay off while the bridge feeds `LEGACY` Product. Deploy validates the persistent handoff before/after release and separates Product relay mode from collector ownership.
