...
- Docker Stage 5 enables only Account 2 processor in live-shadow. It keeps local 3d raw staging/redacted Store and requires atomic immutable feedback/prediction SQLite snapshots; missing/invalid inputs fail closed. It never writes product DB, Facts outbox, or model authority.
- Docker Stage 6 consumes both account spools with separate budgets/all seven streams. XAU keeps every quote; public melted facts are transient 3d. Private offers are immutable, outcomes separate, and routine expiry deletion is not economic retraction. Permanent outcomes wait for Stage 8.
