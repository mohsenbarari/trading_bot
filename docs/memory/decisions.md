...
- 2026-08-23 | Stage 9 was rebuilt on main's design: the durable command is the handoff, acknowledged in-transaction under a lease-token fence, so the Telegram bot-to-bot hop is removed and channel hops drop 3→1. Our addition is the stage-1 wakeup plus the batch-8 claim. The heartbeat table, its migration, and the local-ack flag were dropped as unnecessary; lane liveness is not a precondition.
