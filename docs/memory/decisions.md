...
- 2026-08-17 | Staging Telegram delivery execution is Queue-v1 on the bot host; API/WebApp stay producer-only. Recipient-less outbox residue stops health and uses the audited reconciler; synthetic cleanup removes paired outbox change logs. Never direct SQL/provider calls. Production remains Legacy pending separate owner authorization.
