...
- Queue-v1 is authorized but production deploy is separate. Staging/production central bots, five Publishers, and channels stay distinct; collisions or shared-fleet opt-in block cutover.
...
- Offer `created_at` is the immutable registration-time lifetime anchor. Staging matrices record queue entry separately and handle backlog with an approved test lifetime, never a schema rewrite.
