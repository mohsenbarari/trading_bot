...
The only primary input is the version-verified canonical Market Store. The
reader enables SQLite `query_only`, reads both `market_observations` and its
cold archive, and never creates, upgrades, or writes the database. A legacy
`price_events` database is rejected rather than inferred. Feature time is
`available_at_utc`, so delayed evidence cannot leak backward into an earlier
cutoff.
...
The public coin input is labelled `COIN_REFERENCE:*`: it is an external
generic cash/paper series only.  It is not an Imam/half/quarter/one-gram offer
and cannot prove a product-specific relationship.
...
anchor after converting canonical toman-per-methqal into the project unit. Its
...
Eligible, non-conditional `COIN_*` rows with `event_type=TRADE` and the
canonical project price unit are the primary outcome labels. Coin offers are
never labels. An optional normalized confirmed-trade sidecar may extend
historical coverage; economic duplicates are removed before evaluation.
Private melted flow remains separate from `PUBLIC_FLOW:*`, preventing two
sources from being averaged into one apparent market.
...
`--acknowledge-shadow-only`; it reads `--market-store` through the canonical
read-only verifier and requires every input/output database or artifact path to
be outside the repository. Its optional JSONL
...
The cycle is an explicit operator command only. No systemd service/timer is
shipped, and nothing in application startup imports or schedules it. Promotion
still requires a separate versioned human decision after walk-forward evidence.
