...
| T-103 | P1 | Current staging runs logical Iran and foreign services against shared infrastructure on one host; it does not reproduce independent databases, Redis instances, upload storage, clocks, or network partitions. | Tests pass because shared state masks identity, replication, split-brain, and file-recovery failures. | Build an isolated three-site-like topology with independent state stores and controllable links, latency, loss, DNS, restarts, and clock skew; prohibit production peers/data. |
...
| T-121 | P1 | Current tests and scripts are dominated by two-server cases; there is no deterministic network fault-injection harness for three independent databases and storage planes. | Rare interleavings remain untested and production becomes the first real split-brain/recovery exercise. | Add directional partitions, latency, reordering, duplication, process kill, disk-full, clock-skew, CDN switch, restart, key-rotation, and migration-overlap scenarios with reproducible seeds. |
