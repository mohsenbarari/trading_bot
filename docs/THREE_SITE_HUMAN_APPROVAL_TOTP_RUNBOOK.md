...
policy and the short-lived approval token.
...
- Every token is signed and bound to one exact action, environment, release
  SHA, artifact hash, policy hash, and action-specific bindings.
...
- There is no generic break-glass action. Emergency actions must still be one
  of the explicit allowlisted actions and bind their exact target state.
...
5. Recreate every still-needed inventory, migration, Full Matrix, Gate D,
   Writer/Witness, failover, or failback approval with the new exact-subject
   token. Legacy approval files are historical evidence only.
