...
### Operator gate for live coin-group input

The dashboard home page has a persistent **connect/disconnect** switch for the
live offer/trade stream from the coin groups. It is an estimator input gate,
not a Telegram collector switch: collectors continue reading and storing
events, and the activity page continues to show them while the gate is off.
The gate state is stored in the protected runtime file
`group-live-input-control.json` (override with `--group-live-control`). While
disconnected, newly received group rows are kept in the conversation database
but are excluded from current live anchors and settlement-ratio calculations;
historical group rows before the disconnect boundary, manual operator rows,
external market inputs, training artifacts and prior anchors remain usable.
Reconnecting clears the boundary and immediately refreshes the estimator, so
all rows accumulated while disconnected are applied without replaying or
...
The live estimator also has a bounded online learning ledger in the
...
occur within five minutes; a disconnected prediction can be bridged only once
per commodity/settlement to the first trusted post-reconnect event within
24 hours. At least three evaluated observations are required before any
correction is applied. The residual is clipped to ±3.5%, the applied shift to
±1.5%, and an interval can only stay the same width or widen. Raw predictions,
residuals and evaluation mode are retained for review. This is online residual
calibration, not automatic retraining or promotion of model weights;
`automatic_model_weight_promotion` is always `false`.
