...
PARSER_VERSION = "rules-v6-independent-form-settlement"
...
def parse_abshdh(text: str) -> list[PriceEvent]:
    ...
    for line in lines:
        ...
        melted_match = re.search(
            r"#\s*(?:آبشده|ابشده)\s*(حواله|نقدی|رسمی|امروزی|غیررسمی|جهانی)?",
            compact,
        )
        if melted_match:
...
def should_ignore_message(channel_username: str, text: str, *, is_forwarded: bool = False) -> bool:
