...
DROP INDEX IF EXISTS idx_raw_posts_xau_minute;
CREATE UNIQUE INDEX IF NOT EXISTS idx_raw_posts_xau_15s
    ON raw_posts(
        source_code,
        CAST(strftime('%s', published_at_utc) / 15 AS INTEGER)
    )
...
def upsert_raw_post(
    ...
    *,
    ...
    if source_code == "XAUUSD":
        same_bucket = connection.execute(
            """
            SELECT id FROM raw_posts
            WHERE source_code = 'XAUUSD'
              AND CAST(strftime('%s', published_at_utc) / 15 AS INTEGER)
                  = CAST(strftime('%s', ?) / 15 AS INTEGER)
            """,
            (post.published_at_utc,),
        ).fetchone()
        if same_bucket is not None:
            connection.execute(
                """
                UPDATE raw_posts
