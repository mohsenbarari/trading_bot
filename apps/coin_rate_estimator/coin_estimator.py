    ...
    try:
        ...
        if "confirmed_trades" in tables:
            ...
            rows = connection.execute(
                ...
                (commodity, settlement, trade_form, iso_utc(start), iso_utc(end), minimum_confidence),
            ).fetchall()
            observations.extend(
                {
    ...
    try:
        ...
        rows = [
            ...
            for row in connection.execute(
                ...
                (trade_form, iso_utc(start), iso_utc(end)),
            ).fetchall()
        ]
        if "confirmed_trades" in tables:
            trade_columns = {
...
def historical_market_context(
    ...
    config = SETTLEMENT_CONFIG[settlement]
    melted = select_melted_average(connection, settlement, end)
    ...
    return {
        ...
        # Dollar messages are materially less frequent than ounce/melted
        # ticks. A trailing ten-minute observed window is used for historical
        # features only; live one-minute missing-value semantics are unchanged.
        "usd": select_effective_usd_average(
            connection, settlement, end, seconds=600
        ),
        "usdt": select_usdt_average(connection, end, seconds=600),
            ...
            for commodity in supported:
                ...
                if (
                    ...
                    if historical_group_anchor.get("status") == "OBSERVED":
                        ...
                        anchor_melted = anchor_inputs["melted_gold"].get("average_price")
                    ...
                    if historical_anchor.get("status") == "OBSERVED":
                        ...
                        if anchor_melted is not None and float(anchor_melted) > 0:
                            ...
                            current_usd = inputs["usd"].get("average_price")
                            anchor_usd = anchor_inputs["usd"].get(
                                "average_price"
                            )
                            if (
