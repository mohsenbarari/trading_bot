...
class Chat(Base):
    ...
    pinned_message_id = Column(Integer, ForeignKey("messages.id", name="fk_chats_pinned_message"), nullable=True)
    pinned_message_at = Column(DateTime(timezone=True), nullable=True)
    pinned_message_by_id = Column(Integer, ForeignKey("users.id", name="fk_chats_pinned_message_by"), nullable=True)
    ...
    pinned_message_by = relationship("User", foreign_keys=[pinned_message_by_id])
