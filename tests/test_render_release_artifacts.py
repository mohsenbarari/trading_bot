...
class RenderReleaseArtifactsTests(unittest.TestCase):
    ...
    def test_recovery_nginx_fences_legacy_direct_sync_receiver(self):
        manifest = self.write_file("online.env", self.manifest_content())
        template = str(
        ...
        direct_sync = nginx.split("location = /api/sync/receive", 1)[1].split("}", 1)[0]
        self.assertIn("return 410;", direct_sync)
        self.assertNotIn("proxy_pass", direct_sync)
        self.assertNotIn("allow 65.109.216.187;", nginx)
