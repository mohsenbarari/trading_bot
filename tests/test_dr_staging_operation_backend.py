...
class StagingOperationBackendTests(unittest.TestCase):
    def setUp(self) -> None:
        # Preserve historical parser/state-machine coverage behind an explicit
        # test-only seam. Runtime enforcement is covered below.
...
class StagingOperationBackendRetirementTests(unittest.IsolatedAsyncioTestCase):
    async def test_config_and_constructor_stop_before_secure_reads(self):
        with (
            patch("core.dr_staging_operation_backend._secure_json") as secure_json,
            patch(
                "core.dr_staging_operation_backend.verify_approved_inventory"
            ) as verify,
        ):
    ...
    async def test_direct_runtime_helpers_stop_before_ssh_or_provider_calls(self):
