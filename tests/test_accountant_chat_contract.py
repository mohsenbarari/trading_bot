...
class AccountantChatContractTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_resolve_relation_aware_sender_display_name_uses_identity_map_and_falls_back(self):
        owner = SimpleNamespace(id=201, account_name="owner-201", avatar_file_id="avatar-201", is_deleted=False)
        ...
        resolved = await resolve_relation_aware_sender_display_name(
            db,
            user=SimpleNamespace(id=101, account_name="raw-accountant"),
        )
        direct_resolved = await resolve_direct_sender_display_name(
            db,
            user=SimpleNamespace(id=101, account_name="raw-accountant"),
        )
        fallback = await resolve_relation_aware_sender_display_name(
            FakeDB(values=[]),
            user=SimpleNamespace(id=9, account_name="raw-user"),
        )
        no_db_fallback = await resolve_relation_aware_sender_display_name(
            object(),
            user=SimpleNamespace(id=7, account_name="raw-no-db"),
        )

        self.assertEqual(resolved, "دفتر مالک")
        self.assertEqual(direct_resolved, "دفتر مالک")
