...
spec = importlib.util.spec_from_file_location(
    "sender_handoff",
    Path(__file__).resolve().parents[1] / "scripts/incident_sender_handoff_20260907.py",
)
handoff = importlib.util.module_from_spec(spec)
spec.loader.exec_module(handoff)


class SenderHandoffGuards(unittest.TestCase):
    def configs(self):
        old = {
            "services": {
                handoff.ROLE: {
                ...
                "market-processor": {"image": "untouched-processor"},
    ...
    def test_only_exact_sender_delta_is_allowed(self):
        handoff.validate_config(*self.configs())

    def test_both_capture_and_parser_are_protected(self):
        for role in ("market-processor", "market-capture-account2"):
    ...
    def test_sender_mount_environment_and_resource_drift_are_rejected(self):
