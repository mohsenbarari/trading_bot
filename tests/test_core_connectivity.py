...
class CoreConnectivityTests(unittest.IsolatedAsyncioTestCase):
    def test_iran_connectivity_target_rejects_legacy_peer_resolution(self):
        with patch.object(connectivity.settings, "single_writer_runtime_enabled", False), patch.object(
            connectivity.settings, "foreign_server_url", None
        ), patch.object(connectivity.settings, "peer_server_url", "https://legacy-peer.example/"), patch.object(
            connectivity.settings, "germany_server_url", "https://germany.example/"
        ):
            with self.assertRaises(LegacyDirectFiIrTransportRetiredError):
                connectivity._iran_connectivity_target_url()

    async def test_check_connectivity_does_not_probe_foreign_peer_in_any_runtime_mode(self):
        with patch.object(connectivity.settings, "server_mode", "iran"), patch.object(
            connectivity.settings, "single_writer_runtime_enabled", False
        ), patch.object(connectivity.settings, "foreign_server_url", "https://foreign.example"), patch.object(
            connectivity.settings, "peer_server_url", "https://legacy-peer.example"
        ), patch.object(connectivity.settings, "germany_server_url", "https://germany.example"), patch(
    ...
    async def test_check_connectivity_never_constructs_peer_client_on_iran(self):
