...
class RequestLoggingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_registration_exchange_bearer_stays_in_post_body_and_out_of_access_logs(self):
        app = make_test_app()
    ...
    async def test_retired_pending_registration_path_is_redacted_if_requested(self):
        app = make_test_app()
        raw_token = "REG-0123456789abcdef0123456789abcdef"

        with patch("core.request_logging._logger") as logger:
            response = await call_app(app, "GET", f"/api/auth/pending-registration/{raw_token}")
