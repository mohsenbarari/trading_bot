...
class RenderFencedFiWriterLeaseGuardUnitTests(unittest.TestCase):
    ...
    def _write_config(self) -> None:
        payload = {
            ...
            "runtime_env_sha256": hashlib.sha256(self.runtime_env.read_bytes()).hexdigest(),
            "term_parent_directory": str(self.term_parent),
            "app_local_port": 18001,
            "release_identity": {
                "descriptor": str(self.root / "etc" / "release-identity.json"),
                "authority_public_key": str(
                    self.root / "etc" / "release-identity-authority.pub"
                ),
