...
class BotTradeCreateTextOfferParseFlowTests(unittest.IsolatedAsyncioTestCase):
    async def test_handle_text_offer_handles_non_offer_parse_error_active_cap_and_success(self):
        ...
        with patch("bot.utils.offer_parser.parse_offer_text", new=AsyncMock(return_value=(None, None))):
        ...
        state = SimpleNamespace(get_state=AsyncMock(return_value=None), update_data=AsyncMock(), set_state=AsyncMock())
        with patch("bot.utils.offer_parser.parse_offer_text", new=AsyncMock(return_value=(None, error))), patch(
            "bot.handlers.trade_create._get_offer_suggestion", return_value="HINT"
        ):
            await handle_text_offer(message, state, user=user, bot=SimpleNamespace())
        ...
        state = SimpleNamespace(get_state=AsyncMock(return_value=None), update_data=AsyncMock(), set_state=AsyncMock())
        with patch("bot.utils.offer_parser.parse_offer_text", new=AsyncMock(return_value=(parsed, None))), patch(
            "core.trading_settings.get_trading_settings", return_value=SimpleNamespace(max_active_offers=3)
        ), patch(
