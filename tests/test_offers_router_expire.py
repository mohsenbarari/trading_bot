...
from core.services.offer_expiry_command_receipt_service import OfferExpiryCommandReplayConflict
...
class OffersRouterExpireTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_internal_republish_resolves_by_public_id_and_commits_receipt_with_expiry(self):
        offer = SimpleNamespace(
            id=501,
            user_id=5,
            status=OfferStatus.ACTIVE,
            home_server="foreign",
            offer_public_id="ofr_internal_source",
            republished_offer_id=None,
            republished_offer_public_id=None,
            channel_message_id=None,
        )
        db = FakeDB(execute_results=[FakeExecuteResult([offer]), FakeExecuteResult([901])])
        request = SimpleNamespace(
        ...
        payload = InternalOfferExpireRequest(
            ...
            replacement_offer_public_id="ofr_internal_replacement",
        )
        ...
        with patch("api.routers.offers.verify_internal_signature", return_value=True), patch(
            "api.routers.offers.current_server", return_value="foreign"
        ), patch(
            "core.services.offer_expiry_service.current_server", return_value="foreign"
        ), patch(
            "api.routers.offers.prepare_offer_expiry_command_receipt",
            new=AsyncMock(return_value=(receipt, False)),
        ), patch(
            "api.routers.offers._expire_offer_side_effects", new=AsyncMock()
        ) as side_effects_mock:
            result = await expire_offer_internal(payload, request, db=db)

        self.assertEqual(offer.id, 501)
        self.assertEqual(offer.status, OfferStatus.EXPIRED)
        self.assertEqual(offer.republished_offer_public_id, "ofr_internal_replacement")
        self.assertEqual(offer.republished_offer_id, 901)
        ...
        self.assertEqual(result["command_id"], str(payload.command_id))
        self.assertEqual(result["replacement_offer_public_id"], "ofr_internal_replacement")
        self.assertFalse(result["replayed"])

    async def test_internal_republish_replays_durable_result_after_response_loss(self):
        offer = SimpleNamespace(
            id=501,
            user_id=5,
            ...
            home_server="foreign",
            offer_public_id="ofr_internal_source",
            republished_offer_public_id="ofr_internal_replacement",
            channel_message_id=None,
        )
        db = FakeDB(execute_results=[FakeExecuteResult([offer])])
        request = SimpleNamespace(
            body=AsyncMock(return_value=b"{}"),
            headers={
                "x-source-server": "iran",
        ...
        with patch("api.routers.offers.verify_internal_signature", return_value=True), patch(
            ...
            "api.routers.offers.prepare_offer_expiry_command_receipt",
            new=AsyncMock(return_value=(receipt, True)),
        ), patch(
            "api.routers.offers._expire_offer_side_effects", new=AsyncMock()
        ) as side_effects_mock:
            result = await expire_offer_internal(payload, request, db=db)
        ...
        self.assertEqual(result["outcome_code"], "expired")
        self.assertEqual(offer.status, OfferStatus.EXPIRED)
        db.commit.assert_awaited_once()
        side_effects_mock.assert_awaited_once()

    async def test_internal_republish_links_an_already_expired_offer_without_replaying_expiry_side_effects(self):
        offer = SimpleNamespace(
            id=503,
            user_id=5,
            status=OfferStatus.EXPIRED,
            home_server="foreign",
            offer_public_id="ofr_expired_source",
            republished_offer_id=None,
            republished_offer_public_id=None,
            channel_message_id=777,
        )
        db = FakeDB(execute_results=[FakeExecuteResult([offer]), FakeExecuteResult([])])
        request = SimpleNamespace(
            body=AsyncMock(return_value=b"{}"),
            headers={"x-source-server": "iran", "x-timestamp": "123", "x-signature": "sig", "x-api-key": "key"},
        )
        payload = InternalOfferExpireRequest(
            offer_public_id="ofr_expired_source",
            owner_user_id=5,
            ...
            source_server="iran",
            expire_reason="republished",
            command_id="56af09b7-52b6-4c9e-af7a-b4cda5de9b57",
            idempotency_key="offer-republish:expired",
            replacement_offer_public_id="ofr_expired_replacement",
        )
        receipt = SimpleNamespace(
            command_id=payload.command_id,
            offer_public_id=payload.offer_public_id,
        ...
        self.assertEqual(result["outcome_code"], "linked_inactive")
        self.assertEqual(offer.status, OfferStatus.EXPIRED)
        self.assertEqual(offer.republished_offer_public_id, "ofr_expired_replacement")
        self.assertIsNone(offer.republished_offer_id)
        self.assertEqual(receipt.outcome_code, "linked_inactive")
        db.commit.assert_awaited_once()
        side_effects_mock.assert_not_awaited()

    async def test_internal_republish_rejects_changed_payload_replay(self):
        db = FakeDB()
        request = SimpleNamespace(
            body=AsyncMock(return_value=b"{}"),
            headers={"x-source-server": "iran", "x-timestamp": "123", "x-signature": "sig", "x-api-key": "key"},
        ...
        payload = InternalOfferExpireRequest(
            ...
            command_id="56af09b7-52b6-4c9e-af7a-b4cda5de9b56",
            idempotency_key="offer-republish:stable",
            replacement_offer_public_id="ofr_changed_replacement",
        )
        ...
        with patch("api.routers.offers.verify_internal_signature", return_value=True), patch(
            ...
            "api.routers.offers.prepare_offer_expiry_command_receipt",
            new=AsyncMock(side_effect=OfferExpiryCommandReplayConflict("changed_payload_replay")),
        ):
            with self.assertRaises(HTTPException) as exc_info:
                await expire_offer_internal(payload, request, db=db)

        self.assertEqual(exc_info.exception.status_code, 409)
        db.rollback.assert_awaited_once()
        db.commit.assert_not_awaited()

    async def test_internal_republish_loses_race_to_completed_offer_without_lineage_mutation(self):
        offer = SimpleNamespace(
            id=502,
            user_id=5,
            status=OfferStatus.COMPLETED,
            home_server="foreign",
            offer_public_id="ofr_completed_source",
            republished_offer_id=None,
            republished_offer_public_id=None,
            channel_message_id=None,
        )
        ...
        payload = InternalOfferExpireRequest(
            ...
            expire_reason="republished",
            command_id="56af09b7-52b6-4c9e-af7a-b4cda5de9b56",
            idempotency_key="offer-republish:completed-race",
            replacement_offer_public_id="ofr_completed_replacement",
        )
        receipt = SimpleNamespace(
            command_id=payload.command_id,
            offer_public_id=payload.offer_public_id,
        ...
        self.assertIsNone(offer.republished_offer_public_id)
        db.rollback.assert_awaited_once()
        db.commit.assert_not_awaited()
