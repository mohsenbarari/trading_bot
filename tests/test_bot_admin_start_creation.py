...
class BotAdminStartCreationTests(unittest.IsolatedAsyncioTestCase):
    async def test_start_invitation_creation_requires_admin_role_and_starts_fsm(self):
        state = FakeState()
        ...
        await start_invitation_creation(message, state, user=SimpleNamespace(role=UserRole.MIDDLE_MANAGER))
        ...
        self.assertEqual(state.updated[-1], {"last_prompt_message_id": 91, "inviter_role": UserRole.MIDDLE_MANAGER.value})

    async def test_start_invitation_creation_inline_requires_admin_role_and_starts_fsm(self):
        state = FakeState()
        ...
        state = FakeState()
        await start_invitation_creation_inline(callback, state, user=SimpleNamespace(role=UserRole.MIDDLE_MANAGER))
        self.assertEqual(state.states, [InvitationCreation.awaiting_account_name])
        callback.message.edit_text.assert_awaited_once()
        self.assertEqual(state.updated[-1], {"last_prompt_message_id": 52, "inviter_role": UserRole.MIDDLE_MANAGER.value})
