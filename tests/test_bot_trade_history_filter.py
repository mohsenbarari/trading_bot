...
class BotTradeHistoryFilterTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_filter_trade_history_ignores_telegram_bad_request(self):
        ...
        with patch("bot.handlers.trade_history._ensure_history_profile_access", new=AsyncMock(return_value=True)), patch(
            ...
            new=AsyncMock(return_value=(SimpleNamespace(account_name="t"), [1])),
        ), patch("bot.handlers.trade_history.format_trade_history", return_value="TEXT"), patch(
            "bot.handlers.trade_history.get_trade_history_keyboard", return_value="KB"
        ), patch("bot.handlers.trade_history.TelegramBadRequest", FakeBadRequest):
