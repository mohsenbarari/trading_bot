...
class _FakeS3:
    ...
    def put_bucket_encryption(self, *, Bucket, ServerSideEncryptionConfiguration):  # noqa: N803, ARG002
        self.mutations.append("encryption")
        self.encryption = ServerSideEncryptionConfiguration

    def put_public_access_block(self, *, Bucket, PublicAccessBlockConfiguration):  # noqa: N803, ARG002
        self.mutations.append("public-access-block")
        self.public_access_block = PublicAccessBlockConfiguration
    ...
    def put_object(
        ...
        *,
        ...
        version = "version-1"
        self.objects[(Bucket, Key)] = (payload, dict(Metadata), version, ServerSideEncryption)
        return {"VersionId": version}

    def head_object(self, *, Bucket, Key):  # noqa: N803
        payload, metadata, version, encryption = self.objects[(Bucket, Key)]
        return {
    ...
    def get_object(self, *, Bucket, Key, VersionId):  # noqa: N803
        payload, _metadata, version, _encryption = self.objects[(Bucket, Key)]
...
class HardenThreeSiteStage3ObjectStorageTests(unittest.TestCase):
    ...
    def test_apply_hardens_and_verifies_encrypted_versioned_roundtrip(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(
                result["status"], "hardened-and-encrypted-readback-verified"
            )
            self.assertEqual(fake.mutations, ["encryption", "public-access-block", "lifecycle", "probe"])
