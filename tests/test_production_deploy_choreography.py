        ...
        for command in (
            ...
            with self.subTest(command=command):
                allowed = run_sourced_script(
                    f"""
PRODUCTION_RELEASE_LOCK_PATH="$2"
COMMAND={command!r}
...
printf 'allowed\\n'
""",
                    "/tmp/production-deploy-choreography-test-no-lock",
                )
                self.assertEqual(
                    allowed.returncode,
                    0,
...
guard_production_release_command
printf '%s|%s|%s\n' \
  "$PRODUCTION_MARKET_MAINTENANCE_LOCK_ADOPTED" \
  "$PRODUCTION_RELEASE_LOCK_OWNED" \
  "$(test -f "$PRODUCTION_RELEASE_LOCK_PATH" && printf present)"
...
""",
    ...
    def test_control_release_prepare_rejects_unbound_market_lock(self) -> None:
        with tempfile.TemporaryDirectory(prefix="market-lock-reject-") as temporary:
            ...
            handoff.mkdir(mode=0o700)
            lock = queue / "production-release.lock"
            lock.write_text('{"schema":"unexpected"}\n', encoding="utf-8")
            lock.chmod(0o600)
            result = run_sourced_script(
                """
PRODUCTION_RELEASE_LOCK_DIR="$2/queue"
