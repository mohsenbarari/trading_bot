...
class BotWriterReadinessTests(unittest.TestCase):
    ...
    def test_same_lease_remains_healthy_across_a_normal_expiry_extension(self) -> None:
        base = datetime.now(timezone.utc)
        writer_readiness.write_writer_ready_marker(self.marker, now=base)
        renewed = ValidatedWriterTerm(
            holder_site="webapp_fi",
            writer_epoch=9,
            lease_id="lease-9",
            issued_at=base,
            expires_at=base + timedelta(seconds=90),
            witness_transition_id="transition-9",
        )
        with patch.object(writer_readiness, "require_application_writer_term", return_value=renewed):
            marker = writer_readiness.check_writer_ready_marker(
                self.marker,
                maximum_age_seconds=45,
                now=base + timedelta(seconds=1),
            )

        self.assertEqual(marker["lease_id"], "lease-9")
