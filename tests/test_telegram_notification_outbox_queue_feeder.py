...
from core import telegram_notification_outbox_queue_feeder as feeder
from core import telegram_notification_outbox_worker as legacy_worker
...
class TelegramNotificationOutboxQueueFeederTests(
    ...
    async def test_cycle_refuses_legacy_owner_before_database_touch(self):
        with patch(
            ...
            "core.telegram_notification_outbox_queue_feeder.AsyncSessionLocal"
        ) as session_factory:
            with self.assertRaisesRegex(
                feeder.TelegramNotificationOutboxQueueFeederOwnershipError,
                "telegram_notification_outbox_feeder_is_not_runtime_owner",
            ):
                await feeder.run_telegram_notification_outbox_queue_handoff_cycle()
        session_factory.assert_not_called()

    async def test_legacy_cycle_refuses_queue_owner_before_database_touch(self):
        with patch(
            "core.telegram_notification_outbox_worker."
            "configured_telegram_delivery_runtime",
            ...
            with self.assertRaisesRegex(
                ...
                await legacy_worker.run_telegram_notification_outbox_delivery_cycle()
