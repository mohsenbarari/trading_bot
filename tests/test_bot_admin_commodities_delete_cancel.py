...
class BotAdminCommoditiesDeleteCancelTests(unittest.IsolatedAsyncioTestCase):
    async def test_handle_delete_confirm_delete_yes_and_cancel_fsm(self):
        query = SimpleNamespace(message=SimpleNamespace(edit_text=AsyncMock()), data="comm_delete_7", answer=AsyncMock())
        ...
        with patch("bot.handlers.admin_commodities.update_anchor", new=AsyncMock()) as anchor_mock, patch(
            ...
            await handle_delete_yes(query, user=SimpleNamespace(id=1, role=UserRole.SUPER_ADMIN), state=state)
        ...
        query = SimpleNamespace(bot=SimpleNamespace(), message=SimpleNamespace(chat=SimpleNamespace(id=1)), answer=AsyncMock())
        ...
        with patch("bot.handlers.admin_commodities.clear_state_retain_anchor", new=AsyncMock()) as clear_mock, patch(
            ...
            await handle_cancel_fsm(query, state=state, user=SimpleNamespace(id=1, role=UserRole.SUPER_ADMIN))
        clear_mock.assert_awaited_once_with(state)
