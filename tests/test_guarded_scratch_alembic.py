...
class GuardedScratchAlembicTests(unittest.TestCase):
    ...
    def test_accepts_market_stage8_scratch_target(self):
        with patch.dict(os.environ, {"TRADING_BOT_MIGRATION_MODE": "scratch"}, clear=True):
            target = validate_scratch_database_urls(
                sync_database_url="postgresql+psycopg2://user:pass@db/market_stage8_price_test",
                database_url="postgresql+asyncpg://user:pass@db/market_stage8_price_test",
            )
        self.assertEqual(target.database_name, "market_stage8_price_test")
    ...
    def test_accepts_market_stage12_scratch_target(self):
        with patch.dict(os.environ, {"TRADING_BOT_MIGRATION_MODE": "scratch"}, clear=True):
            target = validate_scratch_database_urls(
                sync_database_url="postgresql+psycopg2://user:pass@db/market_stage12_pagination_test",
                database_url="postgresql+asyncpg://user:pass@db/market_stage12_pagination_test",
            )
        self.assertEqual(target.database_name, "market_stage12_pagination_test")

    def test_accepts_market_stage13_scratch_target(self):
        with patch.dict(os.environ, {"TRADING_BOT_MIGRATION_MODE": "scratch"}, clear=True):
            target = validate_scratch_database_urls(
                sync_database_url="postgresql+psycopg2://user:pass@db/market_stage13_history_test",
                database_url="postgresql+asyncpg://user:pass@db/market_stage13_history_test",
            )
        self.assertEqual(target.database_name, "market_stage13_history_test")

    def test_accepts_market_stage15_scratch_target(self):
        with patch.dict(os.environ, {"TRADING_BOT_MIGRATION_MODE": "scratch"}, clear=True):
            target = validate_scratch_database_urls(
                sync_database_url="postgresql+psycopg2://user:pass@db/market_stage15_idempotency_test",
                database_url="postgresql+asyncpg://user:pass@db/market_stage15_idempotency_test",
            )
        self.assertEqual(target.database_name, "market_stage15_idempotency_test")

    def test_accepts_telegram_queue_stage3_scratch_target(self):
        with patch.dict(os.environ, {"TRADING_BOT_MIGRATION_MODE": "scratch"}, clear=True):
            target = validate_scratch_database_urls(
                sync_database_url="postgresql+psycopg2://user:pass@db/telegram_queue_stage3_foundation_test",
                database_url="postgresql+asyncpg://user:pass@db/telegram_queue_stage3_foundation_test",
            )
