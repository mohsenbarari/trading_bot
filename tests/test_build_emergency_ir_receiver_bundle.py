...
class BuildEmergencyIrReceiverBundleTests(unittest.TestCase):
    ...
    def test_builds_exact_small_bundle_with_only_allowlisted_members(self) -> None:
        with tempfile.TemporaryDirectory(prefix="emergency-ir-receiver-bundle-") as raw:
            ...
            with tarfile.open(output, "r:gz") as archive:
                ...
                self.assertIn("scripts/__init__.py", {member.name for member in archive.getmembers()})
    ...
    def test_builder_rejects_any_interpreter_without_both_required_flags_before_key_or_output_io(self) -> None:
        common = [
            str(MODULE_PATH),
            "--signing-public-key", "/nonexistent/public.key",
            "--output", "/nonexistent/receiver.tar.gz",
        ]
