# api/routers/commodities.py (نسخه نهایی با import تابع مشترک)
...
import schemas
# --- وابستگی‌های جدید را import کنید ---
from .auth import verify_super_admin_or_dev_key # <-- تابع مشترک از auth وارد شد
...
# === تابع وابستگی محلی از اینجا حذف شد ===
# async def verify_super_admin_or_dev_key(...):
# ...

def extract_duplicate_value(error: IntegrityError) -> str | None:
    # ... (کد این تابع بدون تغییر) ...
    detail = str(error.orig); match = re.search(r"\(([^)=]+)\)=\(([^)]+)\)", detail)
    if match: value = match.group(2); return value
    return None

# --- CRUD کالاها (وابستگی‌ها اکنون از auth می‌آیند) ---
...
async def create_commodity(
    ...
    if (await db.execute(stmt_name_check)).scalar_one_or_none(): raise HTTPException(status_code=400, detail=f"نام کالا **'{commodity_in.name}'** از قبل وجود دارد.")
    if commodity_in.aliases:
        ...
        if existing_aliases:
            duplicate_aliases = [f"**'{a.alias}'**" for a in existing_aliases]; raise HTTPException(status_code=400, detail=f"نام‌های مستعار زیر از قبل وجود دارند: {', '.join(duplicate_aliases)}")
    db_commodity = Commodity(name=commodity_in.name); db.add(db_commodity)
    for alias_str in commodity_in.aliases: db_alias = CommodityAlias(alias=alias_str, commodity=db_commodity); db.add(db_alias)
    try:
        await db.commit(); await db.refresh(db_commodity, ["aliases"]); return db_commodity
    except IntegrityError as e:
        await db.rollback(); duplicate_value = extract_duplicate_value(e); error_detail = f"خطای یکپارچگی دیتابیس: مقدار تکراری."
        if duplicate_value:
...
async def update_commodity(
    ...
    if commodity_in.name is not None and commodity_in.name != db_commodity.name:
        ...
        db_commodity.name = commodity_in.name; is_modified = True
    if commodity_in.aliases is not None:
        new_aliases_set = set(commodity_in.aliases); current_aliases_set = {alias.alias for alias in db_commodity.aliases}
        ...
        if aliases_to_add:
            ...
            existing_aliases = (await db.execute(stmt_alias_check)).scalars().all()
            if existing_aliases:
                duplicate_aliases = [f"**'{a.alias}'**" for a in existing_aliases]; raise HTTPException(status_code=400, detail=f"نام‌های مستعار زیر توسط کالاهای دیگر استفاده شده‌اند: {', '.join(duplicate_aliases)}")
    ...
    if is_modified:
         try:
             await db.commit(); await db.refresh(db_commodity, ["aliases"])
         except IntegrityError as e:
             await db.rollback(); duplicate_value = extract_duplicate_value(e); error_detail = f"خطای یکپارچگی دیتابیس: مقدار تکراری."
             if duplicate_value:
                  if 'commodities_name' in str(e.orig): error_detail = f"نام کالا **'{duplicate_value}'** از قبل وجود دارد."
...
async def add_alias_to_commodity(
    commodity_id: int, alias_in: schemas.CommodityAliasCreate, db: AsyncSession = Depends(get_db), _ = Depends(verify_super_admin_or_dev_key)
):
    # ... (کد این تابع بدون تغییر) ...
    db_commodity = (await db.execute(select(Commodity).where(Commodity.id == commodity_id))).scalar_one_or_none()
    if not db_commodity: raise HTTPException(status_code=404, detail="کالا یافت نشد")
    ...
    db_alias = CommodityAlias(alias=alias_in.alias, commodity_id=commodity_id); db.add(db_alias)
    try:
        await db.commit(); await db.refresh(db_alias); return db_alias
    except IntegrityError as e:
        await db.rollback(); duplicate_value = extract_duplicate_value(e)
...
async def update_alias(
    alias_id: int, alias_in: schemas.CommodityAliasUpdate, db: AsyncSession = Depends(get_db), _ = Depends(verify_super_admin_or_dev_key)
):
    # ... (کد این تابع بدون تغییر) ...
    stmt = select(CommodityAlias).where(CommodityAlias.id == alias_id)
    db_alias = (await db.execute(stmt)).scalar_one_or_none()
    if not db_alias: raise HTTPException(status_code=404, detail="نام مستعار یافت نشد")
    if db_alias.alias == alias_in.alias: return db_alias
    ...
    try:
        ...
        await db.rollback(); raise HTTPException(status_code=400, detail=f"نام مستعار **'{alias_in.alias}'** از قبل وجود دارد.")

@router.delete("/aliases/{alias_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_alias(
    alias_id: int, db: AsyncSession = Depends(get_db), _ = Depends(verify_super_admin_or_dev_key)
):
    # ... (کد این تابع بدون تغییر) ...
    stmt = select(CommodityAlias).where(CommodityAlias.id == alias_id)
    db_alias = (await db.execute(stmt)).scalar_one_or_none()
    if not db_alias: raise HTTPException(status_code=404, detail="نام مستعار یافت نشد")
    await db.delete(db_alias); await db.commit()
    return None