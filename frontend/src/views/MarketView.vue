...
const {
  offers,
  isLoading,
  isLoadingMore: activeOffersLoading,
  error: activeOffersError,
  paginationError: activeOffersPaginationError,
  hasMore: hasMoreActiveOffers,
  fetchOffers,
  loadMoreOffers,
  setFilters: setActiveOfferFilters,
  startPolling,
  stopPolling,
} = useOffers()
...
const commodityFilterType = ref('all')
...
const commodityFilterOptions = computed(() => [
  ...
  ...commodities.value.map((commodity) => ({
...
function matchesMarketFilters(offer: any) {
  ...
  if (
    settlementFilterType.value !== 'all'
    && normalizeSettlementType(offer?.settlement_type) !== settlementFilterType.value
  ) return false
  if (
    commodityFilterType.value !== 'all'
    && Number(offer?.commodity_id) !== Number(commodityFilterType.value)
  ) return false
  return true
}

function currentActiveOfferFilters() {
  return {
    offerType: filterType.value === 'buy' || filterType.value === 'sell'
      ? filterType.value
      : undefined,
    settlementType: settlementFilterType.value === 'all'
      ? undefined
      : settlementFilterType.value,
    commodityId: commodityFilterType.value === 'all'
      ? undefined
      : Number(commodityFilterType.value),
    ownOnly: filterType.value === 'my',
  }
}

async function retryActiveOfferLoad() {
  if (activeOffersPaginationError.value && offers.value.length > 0) {
...
watch(
  ...
  () => {
    void setActiveOfferFilters(currentActiveOfferFilters())
  },
  { immediate: true },
)
    ...
    <div class="market-header">
      <div class="header-controls">
        ...
        <div class="market-filter-strip">
          ...
          <span class="market-filter-divider" aria-hidden="true"></span>
          <AppFilterChips v-model="commodityFilterType" class="tabs-container market-commodity-filter-chips" label="فیلتر کالا" :options="commodityFilterOptions" />
        </div>
      </div>
    ...
    <div class="market-content">
      <div class="content-inner">
        <OffersList
          ...
          @load-more-active="loadMoreOffers"
          @retry-active="retryActiveOfferLoad"
...
.market-commodity-filter-chips :deep(.ui-filter-chip:focus-visible),
