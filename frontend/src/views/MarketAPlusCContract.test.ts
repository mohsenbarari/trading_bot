...
describe('Market A+C source contracts', () => {
  ...
  it('binds a linear inset deadline meter and a reduced-motion-safe overtime sticker', () => {
    ...
    expect(offersList).toContain('--history-card-min-block-size: 8.75rem')
    expect(offersList).toContain('min-block-size: var(--history-card-min-block-size)')
    expect(offersList).toContain('0 -8px 18px -8px')
    expect(offersList).toContain('--history-card-min-block-size: 5.1875rem')
