...
describe('Market A+C source contracts', () => {
  ...
  it('binds server-authoritative perimeter progress and a reduced-motion-safe overtime sticker', () => {
    expect(appOfferCard).toContain('data-test="offer-deadline-perimeter"')
    expect(appOfferCard).toContain('offer-deadline-perimeter__value')
    expect(appOfferCard).toContain('stroke-dasharray: var(--t-pct, 100) 100')
    ...
    expect(offersList).not.toContain('offer-deadline-bar')
    expect(offersList).not.toContain('offer-deadline-fill')
