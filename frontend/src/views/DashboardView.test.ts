...
describe('DashboardView.vue', () => {
  ...
  it('loads the current user, shows the unread notification dot, and routes the top-bar actions', async () => {
    ...
    expect(wrapper.get('.dashboard-status-strip').text()).toContain('حساب فعال')
    expect(wrapper.get('.dashboard-status-strip').text()).toContain('آماده انجام عملیات روزانه')
    expect(wrapper.get('.dashboard-status-strip').text()).toContain('بازار: بازار باز')
    expect(wrapper.get('.dashboard-status-strip').text()).toContain('کار امروز: ۱ معامله')
    expect(wrapper.get('.dashboard-status-strip').text()).toContain('۱ اعلان')
