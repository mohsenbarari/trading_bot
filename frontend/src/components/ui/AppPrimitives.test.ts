...
describe('ui primitives', () => {
  ...
  it('renders list items and confirm dialogs with action events', async () => {
    ...
    expect(dialog.get('[role="alert"]').text()).toBe('ارتباط با سرور برقرار نشد.')
    expect(dialog.findAll('button')[0]!.attributes('disabled')).toBeDefined()
    expect(dialog.findAll('button')[1]!.attributes('disabled')).toBeDefined()
    expect(dialog.findAll('button')[1]!.attributes('aria-busy')).toBe('true')
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape' }))
    await dialog.findAll('button')[1]!.trigger('click')
    expect(dialog.emitted('cancel')).toHaveLength(2)
    expect(dialog.emitted('confirm')).toHaveLength(1)
    ...
    expect(dialog.findAll('button')[0]!.attributes('disabled')).toBeUndefined()
    expect(dialog.findAll('button')[1]!.attributes('disabled')).toBeDefined()
    dialog.unmount()
    await nextTick()
