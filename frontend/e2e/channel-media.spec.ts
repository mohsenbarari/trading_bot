...
test.describe('Channel media regressions', () => {
  ...
  test('share receive can fan out shared text to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct', 'member')
    const shareKey = `pw-share-group-direct-${Date.now()}`
    const shareTitle = `Playwright Shared Group+Direct Title ${Date.now()}`
    const shareText = `Playwright Shared Group+Direct Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-direct/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group+Direct ${Date.now()}`
    const directSeedContent = `PW DIRECT GROUP TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()

    await seedDirectTextMessage(request, fixture, directSeedContent)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      title: shareTitle,
  ...
  test('share receive can fan out shared text to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel', 'admin')
    const shareKey = `pw-share-group-channel-${Date.now()}`
    const shareTitle = `Playwright Shared Group+Channel Title ${Date.now()}`
    const shareText = `Playwright Shared Group+Channel Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-channel/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group+Channel ${Date.now()}`
    ...
    const groupBootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(groupBootstrapResponse.ok()).toBeTruthy()

    await seedBootstrapChannelMessage(request, fixture, `PLAYWRIGHT SHARE RECEIVE CHANNEL GROUP ${Date.now()}`)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      title: shareTitle,
  ...
  test('share receive can fan out shared document and image uploads to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_files', 'member')
    const shareKey = `pw-share-group-direct-files-${Date.now()}`
    const groupTitle = `Playwright Share Group Files ${Date.now()}`
    const sharedDocumentName = `pw-share-group-direct-${Date.now()}.txt`
    const sharedImageName = `pw-share-group-direct-${Date.now()}.png`
    const sharedDocumentBody = `PW SHARE GROUP DIRECT FILE BODY ${Date.now()}`
    const directSeedContent = `PW DIRECT GROUP FILE TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT FILES ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    const [groupContents, directContents] = await Promise.all([
      fetchLatestRoomContentsByChatId(request, fixture.accessToken, groupId),
      fetchLatestDirectContents(request, fixture, fixture.creatorUserId),
    ])

    const groupDocumentPayload = extractSharedFilePayload(groupContents, sharedDocumentName)
    ...
    expect(groupDocumentPayload?.file_id).toBeTruthy()
    expect(directDocumentPayload?.file_id).toBeTruthy()
    expect(groupDocumentPayload?.file_id).toBe(directDocumentPayload?.file_id)

    expect(groupImagePayload?.file_id).toBeTruthy()
    expect(directImagePayload?.file_id).toBeTruthy()
    expect(groupImagePayload?.file_id).toBe(directImagePayload?.file_id)
  })

  test('share receive can fan out one shared document upload to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_file', 'member')
    const shareKey = `pw-share-group-direct-file-${Date.now()}`
    const groupTitle = `Playwright Share Group Direct File ${Date.now()}`
    const sharedDocumentName = `pw-share-group-direct-file-${Date.now()}.txt`
    const sharedDocumentBody = `PW SHARE GROUP DIRECT FILE BODY ${Date.now()}`
    const directSeedContent = `PW DIRECT GROUP FILE TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT FILE ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    expect(groupPayload?.file_id).toBeTruthy()
    expect(directPayload?.file_id).toBeTruthy()
    expect(groupPayload?.file_id).toBe(directPayload?.file_id)
  })

  test('share receive can fan out shared text and one shared document to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_text_file', 'member')
    const shareKey = `pw-share-group-direct-text-file-${Date.now()}`
    const shareTitle = `Playwright Shared Group Direct Mixed Title ${Date.now()}`
    const shareText = `Playwright Shared Group Direct Mixed Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-direct-text-file/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Direct Mixed ${Date.now()}`
    const sharedDocumentName = `pw-share-group-direct-text-file-${Date.now()}.txt`
    const sharedDocumentBody = `PW SHARE GROUP DIRECT TEXT FILE BODY ${Date.now()}`
    const directSeedContent = `PW DIRECT GROUP TEXT FILE TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT TEXT FILE ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
  ...
  test('share receive can fan out shared text and one shared image to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_text_image', 'member')
    const shareKey = `pw-share-group-direct-text-image-${Date.now()}`
    const shareTitle = `Playwright Shared Group Direct Image Title ${Date.now()}`
    const shareText = `Playwright Shared Group Direct Image Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-direct-text-image/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Direct Image ${Date.now()}`
    const sharedImageName = `pw-share-group-direct-text-image-${Date.now()}.png`
    const directSeedContent = `PW DIRECT GROUP TEXT IMAGE TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT TEXT IMAGE ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    const groupPayload = extractSharedFilePayload(groupContents, sharedImageName)
  ...
  test('share receive can fan out shared text and one shared video to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_text_video', 'member')
    const shareKey = `pw-share-group-direct-text-video-${Date.now()}`
    const shareTitle = `Playwright Shared Group Direct Video Title ${Date.now()}`
    const shareText = `Playwright Shared Group Direct Video Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-direct-text-video/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Direct Video ${Date.now()}`
    const sharedVideoName = `pw-share-group-direct-text-video-${Date.now()}.webm`
    const directSeedContent = `PW DIRECT GROUP TEXT VIDEO TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT TEXT VIDEO ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    const groupPayload = extractSharedFilePayload(groupContents, sharedVideoName)
  ...
  test('share receive can fan out shared text, image, and video to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_text_image_video', 'member')
    const shareKey = `pw-share-group-direct-text-image-video-${Date.now()}`
    const shareTitle = `Playwright Shared Group Direct Image Video Title ${Date.now()}`
    const shareText = `Playwright Shared Group Direct Image Video Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-direct-text-image-video/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Direct Image Video ${Date.now()}`
    const sharedImageName = `pw-share-group-direct-text-image-video-${Date.now()}.png`
    const sharedVideoName = `pw-share-group-direct-text-image-video-${Date.now()}.webm`
    const directSeedContent = `PW DIRECT GROUP TEXT IMAGE VIDEO TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT TEXT IMAGE VIDEO ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    const directVideoPayload = extractSharedFilePayload(directContents, sharedVideoName)
    ...
    expect(directVideoPayload?.file_id).toBeTruthy()
    expect(groupVideoPayload?.file_id).toBe(directVideoPayload?.file_id)
  })

  test('share receive can fan out shared document, image, and video uploads to a group and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_direct_three_files', 'member')
    const shareKey = `pw-share-group-direct-three-files-${Date.now()}`
    const groupTitle = `Playwright Share Group Direct Three Files ${Date.now()}`
    const sharedDocumentName = `pw-share-group-direct-three-files-${Date.now()}.txt`
    const sharedImageName = `pw-share-group-direct-three-files-${Date.now()}.png`
    const sharedVideoName = `pw-share-group-direct-three-files-${Date.now()}.webm`
    const sharedDocumentBody = `PW SHARE GROUP DIRECT THREE FILE BODY ${Date.now()}`
    const directSeedContent = `PW DIRECT GROUP THREE FILE TARGET SEED ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP DIRECT THREE FILES ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    await expect
      .poll(async () => fetchLatestRoomMessageTypesByChatId(request, fixture.accessToken, groupId), { timeout: 30000 })
      .toEqual(expect.arrayContaining(['document', 'image', 'video']))
    await expect
      .poll(async () => fetchLatestDirectMessageTypes(request, fixture, fixture.creatorUserId), { timeout: 30000 })
      .toEqual(expect.arrayContaining(['document', 'image', 'video']))
    await expect
  ...
  test('share receive can fan out shared document and image uploads to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel_files', 'admin')
    const shareKey = `pw-share-group-channel-files-${Date.now()}`
    const groupTitle = `Playwright Share Group Channel Files ${Date.now()}`
    const sharedDocumentName = `pw-share-group-channel-${Date.now()}.txt`
    const sharedImageName = `pw-share-group-channel-${Date.now()}.png`
    const sharedDocumentBody = `PW SHARE GROUP CHANNEL FILE BODY ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL FILES ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()
    ...
    expect(groupDocumentPayload?.file_id).toBe(channelDocumentPayload?.file_id)
  ...
  test('share receive can fan out shared text and one shared document to a channel and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_multi_text_file', 'admin')
    const bootstrapContent = `PLAYWRIGHT SHARE RECEIVE MULTI TEXT FILE ${Date.now()}`
    const seedDirectContent = `PW DIRECT TEXT FILE TARGET SEED ${Date.now()}`
    const shareKey = `pw-share-multi-text-file-${Date.now()}`
    const shareTitle = `Playwright Shared Mixed Title ${Date.now()}`
    const shareText = `Playwright Shared Mixed Body ${Date.now()}`
    const shareUrl = `https://example.test/share/multi-text-file/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const sharedDocumentName = `pw-share-multi-text-file-${Date.now()}.txt`
    const sharedDocumentBody = `PW SHARE MULTI TEXT FILE BODY ${Date.now()}`
    const channelTarget = page.locator('.forward-target-item').filter({ hasText: fixture.channelTitle })
    const directTarget = page.locator('.forward-target-item').filter({ hasText: fixture.creatorAccountName })
  ...
  test('share receive can fan out shared text and one shared document to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel_text_file', 'admin')
    const shareKey = `pw-share-group-channel-text-file-${Date.now()}`
    const shareTitle = `Playwright Shared Group Channel Mixed Title ${Date.now()}`
    const shareText = `Playwright Shared Group Channel Mixed Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-channel-text-file/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Channel Mixed ${Date.now()}`
    const sharedDocumentName = `pw-share-group-channel-text-file-${Date.now()}.txt`
    const sharedDocumentBody = `PW SHARE GROUP CHANNEL TEXT FILE BODY ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL TEXT FILE ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()

    await seedBootstrapChannelMessage(request, fixture, `PLAYWRIGHT SHARE RECEIVE CHANNEL TEXT FILE GROUP ${Date.now()}`)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      title: shareTitle,
  ...
  test('share receive can fan out shared text and one shared image to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel_text_image', 'admin')
    const shareKey = `pw-share-group-channel-text-image-${Date.now()}`
    const shareTitle = `Playwright Shared Group Channel Image Title ${Date.now()}`
    const shareText = `Playwright Shared Group Channel Image Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-channel-text-image/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Channel Image ${Date.now()}`
    const sharedImageName = `pw-share-group-channel-text-image-${Date.now()}.png`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL TEXT IMAGE ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()

    await seedBootstrapChannelMessage(request, fixture, `PLAYWRIGHT SHARE RECEIVE CHANNEL TEXT IMAGE GROUP ${Date.now()}`)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      title: shareTitle,
  ...
  test('share receive can fan out shared text and one shared video to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel_text_video', 'admin')
    const shareKey = `pw-share-group-channel-text-video-${Date.now()}`
    const shareTitle = `Playwright Shared Group Channel Video Title ${Date.now()}`
    const shareText = `Playwright Shared Group Channel Video Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-channel-text-video/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Channel Video ${Date.now()}`
    const sharedVideoName = `pw-share-group-channel-text-video-${Date.now()}.webm`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL TEXT VIDEO ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()

    await seedBootstrapChannelMessage(request, fixture, `PLAYWRIGHT SHARE RECEIVE CHANNEL TEXT VIDEO GROUP ${Date.now()}`)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      title: shareTitle,
  ...
  test('share receive can fan out shared document, image, and video uploads to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel_three_files', 'admin')
    const shareKey = `pw-share-group-channel-three-files-${Date.now()}`
    const groupTitle = `Playwright Share Group Channel Three Files ${Date.now()}`
    const sharedDocumentName = `pw-share-group-channel-three-files-${Date.now()}.txt`
    const sharedImageName = `pw-share-group-channel-three-files-${Date.now()}.png`
    const sharedVideoName = `pw-share-group-channel-three-files-${Date.now()}.webm`
    const sharedDocumentBody = `PW SHARE GROUP CHANNEL THREE FILE BODY ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL THREE FILES ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()

    await seedBootstrapChannelMessage(request, fixture, `PLAYWRIGHT SHARE RECEIVE CHANNEL THREE FILES GROUP ${Date.now()}`)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      files: [
  ...
  test('share receive can fan out shared text, document, image, and video to a group and a channel target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_group_channel_full_payload', 'admin')
    const shareKey = `pw-share-group-channel-full-payload-${Date.now()}`
    const shareTitle = `Playwright Shared Group Channel Full Title ${Date.now()}`
    const shareText = `Playwright Shared Group Channel Full Body ${Date.now()}`
    const shareUrl = `https://example.test/share/group-channel-full-payload/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const groupTitle = `Playwright Share Group Channel Full Payload ${Date.now()}`
    const sharedDocumentName = `pw-share-group-channel-full-payload-${Date.now()}.txt`
    const sharedImageName = `pw-share-group-channel-full-payload-${Date.now()}.png`
    const sharedVideoName = `pw-share-group-channel-full-payload-${Date.now()}.webm`
    const sharedDocumentBody = `PW SHARE GROUP CHANNEL FULL PAYLOAD BODY ${Date.now()}`
    ...
    const bootstrapResponse = await request.post(`${BACKEND_BASE_URL}/api/chat/rooms/${groupId}/send`, {
      headers: authHeaders(fixture.creatorAccessToken),
      data: {
        content: `PLAYWRIGHT SHARE RECEIVE GROUP CHANNEL FULL PAYLOAD ${Date.now()}`,
        message_type: 'text',
      },
    })
    expect(bootstrapResponse.ok()).toBeTruthy()

    await seedBootstrapChannelMessage(request, fixture, `PLAYWRIGHT SHARE RECEIVE CHANNEL FULL PAYLOAD GROUP ${Date.now()}`)
    await loginWithSeededSession(page, fixture)
    await seedShareReceivePayload(page, {
      key: shareKey,
      title: shareTitle,
    ...
    await expect
      .poll(async () => fetchLatestRoomMessageTypesByChatId(request, fixture.accessToken, groupId), { timeout: 30000 })
      .toEqual(expect.arrayContaining(['text', 'document', 'image', 'video']))
    await expect
      .poll(async () => fetchLatestRoomMessageTypes(request, fixture), { timeout: 30000 })
      .toEqual(expect.arrayContaining(['text', 'document', 'image', 'video']))
    await expect
  ...
  test('share receive can fan out shared text and one shared image to a channel and a direct chat target', async ({
    ...
    const fixture = seedChannelSession('share_receive_multi_text_image', 'admin')
    const bootstrapContent = `PLAYWRIGHT SHARE RECEIVE MULTI TEXT IMAGE ${Date.now()}`
    const seedDirectContent = `PW DIRECT TEXT IMAGE TARGET SEED ${Date.now()}`
    const shareKey = `pw-share-multi-text-image-${Date.now()}`
    const shareTitle = `Playwright Shared Image Title ${Date.now()}`
    const shareText = `Playwright Shared Image Body ${Date.now()}`
    const shareUrl = `https://example.test/share/multi-text-image/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const sharedImageName = `pw-share-multi-text-image-${Date.now()}.png`
    const channelTarget = page.locator('.forward-target-item').filter({ hasText: fixture.channelTitle })
    const directTarget = page.locator('.forward-target-item').filter({ hasText: fixture.creatorAccountName })
  ...
  test('share receive can fan out shared text, image, and video to a channel and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_multi_text_image_video', 'admin')
    const bootstrapContent = `PLAYWRIGHT SHARE RECEIVE MULTI TEXT IMAGE VIDEO ${Date.now()}`
    const seedDirectContent = `PW DIRECT TEXT IMAGE VIDEO TARGET SEED ${Date.now()}`
    const shareKey = `pw-share-multi-text-image-video-${Date.now()}`
    const shareTitle = `Playwright Shared Image Video Title ${Date.now()}`
    const shareText = `Playwright Shared Image Video Body ${Date.now()}`
    const shareUrl = `https://example.test/share/multi-text-image-video/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const sharedImageName = `pw-share-multi-text-image-video-${Date.now()}.png`
    const sharedVideoName = `pw-share-multi-text-image-video-${Date.now()}.webm`
    const channelTarget = page.locator('.forward-target-item').filter({ hasText: fixture.channelTitle })
    const directTarget = page.locator('.forward-target-item').filter({ hasText: fixture.creatorAccountName })
    ...
    expect(channelVideoPayload?.file_id).toBeTruthy()
    expect(directVideoPayload?.file_id).toBeTruthy()
    expect(channelVideoPayload?.file_id).toBe(directVideoPayload?.file_id)
  })

  test('share receive can fan out shared document, image, and video uploads to a channel and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_multi_three_files', 'admin')
    const bootstrapContent = `PLAYWRIGHT SHARE RECEIVE MULTI THREE FILES ${Date.now()}`
    const seedDirectContent = `PW DIRECT THREE FILE TARGET SEED ${Date.now()}`
    const shareKey = `pw-share-multi-three-files-${Date.now()}`
    const sharedDocumentName = `pw-share-multi-three-files-${Date.now()}.txt`
    const sharedImageName = `pw-share-multi-three-files-${Date.now()}.png`
    const sharedVideoName = `pw-share-multi-three-files-${Date.now()}.webm`
    const sharedDocumentBody = `PW SHARE MULTI THREE FILE BODY ${Date.now()}`
    const channelTarget = page.locator('.forward-target-item').filter({ hasText: fixture.channelTitle })
    const directTarget = page.locator('.forward-target-item').filter({ hasText: fixture.creatorAccountName })
  ...
  test('share receive can fan out shared text, document, image, and video to a channel and a direct chat target', async ({
    page,
    request,
  }) => {
    const fixture = seedChannelSession('share_receive_multi_full_payload', 'admin')
    const bootstrapContent = `PLAYWRIGHT SHARE RECEIVE MULTI FULL PAYLOAD ${Date.now()}`
    const seedDirectContent = `PW DIRECT FULL PAYLOAD TARGET SEED ${Date.now()}`
    const shareKey = `pw-share-multi-full-payload-${Date.now()}`
    const shareTitle = `Playwright Shared Full Title ${Date.now()}`
    const shareText = `Playwright Shared Full Body ${Date.now()}`
    const shareUrl = `https://example.test/share/multi-full-payload/${Date.now()}`
    const expectedMergedText = `${shareTitle}\n${shareText}\n${shareUrl}`
    const sharedDocumentName = `pw-share-multi-full-payload-${Date.now()}.txt`
    const sharedImageName = `pw-share-multi-full-payload-${Date.now()}.png`
    const sharedVideoName = `pw-share-multi-full-payload-${Date.now()}.webm`
    const sharedDocumentBody = `PW SHARE MULTI FULL PAYLOAD BODY ${Date.now()}`
    const channelTarget = page.locator('.forward-target-item').filter({ hasText: fixture.channelTitle })
    const directTarget = page.locator('.forward-target-item').filter({ hasText: fixture.creatorAccountName })
