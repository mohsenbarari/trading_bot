...
try {
  ...
  } else if (market.kind === MARKET_A_PLUS_C_LINEAR_METER_KIND) {
    console.log(
      `PASS Market A+C linear deadline/terminal-clarity disposition (exact ${MARKET_A_PLUS_C_LINEAR_METER_ALLOWED_PATHS.length}-file overlay; ${market.evidence.count} files, ${market.evidence.contentBytes} bytes, ${market.evidence.pathSetSha256}, ${market.evidence.sha256})`,
    )
  ...
  } else if (market.kind === MARKET_OVERTIME_REQUESTER_ACK_KIND) {
    console.log(
      `PASS Market overtime requester local-acknowledgement disposition (exact ${MARKET_OVERTIME_REQUESTER_ACK_ALLOWED_PATHS.length}-file overlay; ${market.evidence.count} files, ${market.evidence.contentBytes} bytes, ${market.evidence.pathSetSha256}, ${market.evidence.sha256})`,
    )
  ...
  } else if (market.kind === MARKET_HISTORY_UNIFORM_LIVE_KIND) {
    console.log(
      `PASS Market uniform live-history disposition (exact ${MARKET_HISTORY_UNIFORM_LIVE_ALLOWED_PATHS.length}-file overlay; ${market.evidence.count} files, ${market.evidence.contentBytes} bytes, ${market.evidence.pathSetSha256}, ${market.evidence.sha256})`,
    )
  } else {
