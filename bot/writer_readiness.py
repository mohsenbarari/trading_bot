...
def check_writer_ready_marker(
    ...
    *,
    ...
    marker_expiry = _parse_timestamp(marker["lease_expires_at"], label="lease expiry")
    # Witness renewal preserves epoch/lease_id while extending expiry.  An
    # already authenticated bot must not flap unhealthy for up to one polling
    # interval after each normal renewal; the live term check above proves the
    # current expiry, while a marker that claims a later expiry is rejected.
    if marker_expiry > term.expires_at:
        raise BotWriterReadinessError("bot readiness marker lease expiry exceeds the live term")
