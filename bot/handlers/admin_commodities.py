...
def _interaction_event_kwargs(event: types.Message | types.CallbackQuery) -> dict:
    if (
        ...
        return {"interaction_event": event}
    return {}
...
async def safe_delete_message(bot: Bot, chat_id: int, message_id: int, delay: int = 0):
    ...
    if (
        ...
        # Queue mode deliberately preserves navigation/anchor messages.  A
        # future explicit cleanup policy may enqueue a bounded durable delete.
        return
    if delay > 0:
...
async def update_anchor(state: FSMContext, new_message_id: int | None, bot: Bot, chat_id: int):
    """
    ...
    if new_message_id is None:
        raise ValueError("legacy_anchor_message_id_missing")
...
async def show_commodity_list(
    bot: Bot,
    chat_id: int,
    user: User,
    state: FSMContext,
    interaction_event: types.Message | types.CallbackQuery | None = None,
):
    ...
    try:
        ...
        if (
            ...
            if interaction_event is None:
                raise ValueError("commodity_queue_interaction_context_missing")
            adapter = (
                answer_callback_message_via_runtime
...
async def handle_manage_commodities(message: types.Message, user: Optional[User], state: FSMContext):
    ...
    await show_commodity_list(
        ...
        user,
        state,
        **_interaction_event_kwargs(message),
    )
...
async def handle_back_to_list(query: types.CallbackQuery, user: Optional[User], state: FSMContext):
    ...
    await show_commodity_list(
        ...
        user,
        state,
        **_interaction_event_kwargs(query),
    )
...
async def handle_alias_add_name(message: types.Message, state: FSMContext, user: Optional[User]):
    ...
    status_msg = await answer_incoming_message_via_runtime(message, user, f"⏳ در حال افزودن **'{new_alias_name}'**...", parse_mode="Markdown")
    ...
    try:
        ...
        await edit_interaction_result_via_runtime(message, user, status_msg, f"❌ خطا: {detail}", parse_mode="Markdown")
        return
    except Exception as e:
        await edit_interaction_result_via_runtime(message, user, status_msg, f"❌ خطای سیستمی: {e}", parse_mode="Markdown")
        return
...
async def handle_alias_edit_name(message: types.Message, state: FSMContext, user: Optional[User]):
    ...
    status_msg = await answer_incoming_message_via_runtime(message, user, f"⏳ در حال ویرایش...", parse_mode="Markdown")
    await update_anchor(state, status_msg.message_id, message.bot, message.chat.id)
    await clear_state_retain_anchor(state)
    ...
    try:
        ...
        await edit_interaction_result_via_runtime(message, user, status_msg, f"❌ خطا: {e}", parse_mode="Markdown")
...
async def handle_alias_delete_yes(query: types.CallbackQuery, user: Optional[User], state: FSMContext):
    ...
    status_msg = await edit_callback_message_via_runtime(query, user, "⏳ در حال حذف...", reply_markup=None)
...
async def handle_commodity_edit_name(message: types.Message, state: FSMContext, user: Optional[User]):
    ...
    status_msg = await answer_incoming_message_via_runtime(message, user, f"⏳ در حال تغییر نام...", parse_mode="Markdown")
    await update_anchor(state, status_msg.message_id, message.bot, message.chat.id)
    await clear_state_retain_anchor(state)
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if any(isinstance(c, dict) and c.get("name") == name for c in all_commodities):
                error_msg = await answer_incoming_message_via_runtime(message, user,
...
async def handle_add_aliases_and_create(message: types.Message, state: FSMContext, user: Optional[User]):
    ...
    status_msg = await answer_incoming_message_via_runtime(message, user, f"⏳ در حال ثبت کالا...", parse_mode="Markdown")
    await update_anchor(state, status_msg.message_id, message.bot, message.chat.id)
...
async def handle_delete_yes(query: types.CallbackQuery, user: Optional[User], state: FSMContext):
    ...
    status_msg = await edit_callback_message_via_runtime(query, user, "⏳ در حال حذف کالا...", reply_markup=None)
    await update_anchor(state, status_msg.message_id, query.bot, query.message.chat.id)
    await clear_state_retain_anchor(state)
