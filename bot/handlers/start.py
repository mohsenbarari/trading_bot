# bot/handlers/start.py (نسخه نهایی و اصلاح شده)
...
from bot.keyboards import get_share_contact_keyboard

router = Router()

# Handler برای /start <token>
...
async def handle_start_with_token(message: types.Message, command: CommandObject, state: FSMContext, user: Optional[User]):
    if user:
        await message.answer("شما قبلاً در این بات ثبت‌نام کرده‌اید. برای دسترسی به پنل از دستور /panel استفاده کنید.")
        return
    ...
    async with AsyncSessionLocal() as session:
        ...
        await message.answer(
            "✅ لینک دعوت شما معتبر است.\n"
            "لطفاً برای تکمیل ثبت‌نام و تایید هویت، روی دکمه زیر کلیک کرده و شماره تماس خود را به اشتراک بگذارید.",
            reply_markup=get_share_contact_keyboard()
        )

# Handler برای /start خالی
@router.message(CommandStart(deep_link=False))
async def handle_start_without_token(message: types.Message, user: Optional[User]):
    if user:
        await message.answer(f"سلام {user.full_name}! به پنل کاربری خود خوش آمدید. برای دسترسی به امکانات از دستور /panel استفاده کنید.")
    else:
        # این پیام توسط handler پیش‌فرض (default.py) مدیریت خواهد شد
        pass

# --- بخش اصلاح شده ---
@router.message(Registration.awaiting_contact, F.contact)
async def handle_contact(message: types.Message, state: FSMContext):
    # ... (بخش اول تابع بدون تغییر) ...
    ...
    async with AsyncSessionLocal() as session:
        ...
        new_user = User(
            ...
            mobile_number=invitation.mobile_number, # <-- فیلد جدید اینجا تنظیم می‌شود
            role=UserRole.WATCH,
            has_bot_access=True
        )
        ...
        await message.answer(
            ...
            "برای دسترسی به امکانات، از دستور /panel استفاده کنید.",
            reply_markup=types.ReplyKeyboardRemove()
        )