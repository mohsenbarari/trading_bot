...
| `candidate/market-recent-offers-chatbox` | Fixes the market chatbox recent-offer repeat list so staging can validate the user's last three expired offers from the past hour while excluding completed/traded offers. | Allowed after focused staging validation and explicit approval |
