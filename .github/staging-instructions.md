...
| `candidate/market-recent-offers-chatbox` | Fixes the market chatbox recent-offer repeat list so staging can validate the last three user offers from the past hour without status narrowing. | Allowed after focused staging validation and explicit approval |
...
| 2026-06-17 | Codex | Added `candidate/market-recent-offers-chatbox` for the market chatbox recent-offer repeat-list fix, branched from current `main` after the trading candidate merge. |
