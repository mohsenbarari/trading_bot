    ...
    *,
    ...
    if current_server() != SERVER_IRAN:
        logger.warning(
            ...
            extra={
                "event": "telegram_otp.forward_attempt",
    ...
    # The promoted reverse path is Object Storage + Witness, never a direct
    # Iran-to-FI HTTP provider command.
    try:
        assert_legacy_direct_fi_ir_transport_retired(
            component="telegram-otp-transport",
            operation="direct IR-to-FI Telegram OTP delivery command",
        )
    except LegacyDirectFiIrTransportRetiredError:
        logger.warning(
            "Telegram OTP direct transport is retired",
            extra={
                "event": "telegram_otp.direct_transport_retired",
                "target_server": SERVER_FOREIGN,
            },
        )
