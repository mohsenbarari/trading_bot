...
def _safe_gate_message(reason: str) -> str:
    ...
    if reason == "legacy_direct_transport_retired":
        return "پایش مستقیم سرور ایران بازنشسته شده است؛ دعوت مشتری تا استقرار مسیر امن جدید در دسترس نیست."
...
async def _fetch_iran_sync_health() -> tuple[dict[str, Any] | None, str | None]:
    # This former FI->IR health request fed a business mutation decision. It
    # is not an approved Object-Storage pull, so reject it before peer URL,
    # observability key, or HTTP client construction.
    try:
        assert_legacy_direct_fi_ir_transport_retired(
            component="customer-invite-sync-gate",
            operation="direct peer sync-health query",
        )
...
async def check_customer_invite_sync_ready(
    *,
    ...
    # The public gate must be equally deterministic: no local queue polling
    # or direct peer health probe can make the retired command plane safe.
    try:
        assert_legacy_direct_fi_ir_transport_retired(
            component="customer-invite-sync-gate",
            operation="customer-invite direct sync readiness check",
        )
    except LegacyDirectFiIrTransportRetiredError:
