...
def required_freshness_actions_for_lane(
    ...
    if identity in TELEGRAM_PUBLISHER_IDENTITIES:
        # Publisher lanes own both their offer-channel lifecycle work and the
        # deadline-bound callback answers received by that same bot.  The
        # owner-aware validators remain the provider-call gate; this exact
        # union prevents publisher tokens from claiming unrelated private or
        # admin queue traffic while keeping callback ownership executable.
        return frozenset(OFFER_FRESHNESS_ACTIONS | CALLBACK_FRESHNESS_ACTIONS)
...
def repeat_offer_response_freshness_routes(
    validator: TelegramDeliveryFreshnessValidator,
) -> dict[TelegramDeliveryAction, TelegramDeliveryFreshnessValidatorCallable]:
    if not callable(validator):
        raise TelegramDeliveryFreshnessRoutingError(
            "telegram_repeat_offer_response_freshness_validator_invalid"
        )
    return {
