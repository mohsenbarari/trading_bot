...
async def fetch_remote_session_authority(target_server: str, user_id: int) -> tuple[int, dict[str, Any]]:
    ...
    # Session transfer has no approved peer-command transport yet.  Deny
    # before URL/payload/HMAC/client construction rather than silently
    # reviving the old home-server HTTP route.
    try:
        assert_legacy_direct_fi_ir_transport_retired(
            component="session-authority",
            operation="direct peer session-authority query",
        )
    except LegacyDirectFiIrTransportRetiredError:
        logger.warning(
            "Remote session authority transport is retired",
            extra={"event": "session.authority.direct_transport_retired", "target_server": normalize_server(target_server, default="")},
        )
