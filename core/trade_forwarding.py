...
async def forward_trade_to_home_server(
    ...
    *,
    ...
    # Object-Storage pull is the only cross-site data path.  Keep this at the
    # very start: a stale deployment must not even resolve a peer or sign a
    # business command when a feature flag is changed locally.
    try:
        assert_legacy_direct_fi_ir_transport_retired(
            component="trade-forwarding",
            operation="direct peer trade command",
        )
    except LegacyDirectFiIrTransportRetiredError:
        logger.warning(
            "trade_forward.direct_transport_retired",
            extra={"event": "trade_forward.direct_transport_retired", "target_server": normalize_server(target_server, default="")},
        )
