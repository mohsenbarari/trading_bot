...
async def forward_customer_invite_to_iran(
    ...
    *,
    ...
    # protocol before they can cross sites.  Do not construct the old HTTP
    # payload as a fallback.
    try:
        assert_legacy_direct_fi_ir_transport_retired(
            component="customer-invite-forwarding",
            operation="direct peer customer-invite command",
        )
    except LegacyDirectFiIrTransportRetiredError:
        logger.warning(
            "Customer invite direct transport is retired",
            extra={"event": "customer_invite.forward.direct_transport_retired", "target_server": SERVER_IRAN},
        )
