"""Queue-mode feeder for project-user membership notification outbox rows."""
...
class TelegramNotificationOutboxQueueFeederOwnershipError(RuntimeError):
    """Raised before DB access when queue-v1 is not the execution owner."""


@dataclass(frozen=True, slots=True)
class TelegramNotificationOutboxQueueFeederReport:
    processed_count: int
...
def _assert_queue_owner() -> None:
    ...
    if (
        ...
        raise TelegramNotificationOutboxQueueFeederOwnershipError(
            "telegram_notification_outbox_feeder_is_not_runtime_owner"
        )
...
async def run_telegram_notification_outbox_queue_handoff_cycle(
    *,
    limit: int | None = None,
) -> TelegramNotificationOutboxQueueFeederReport:
    ...
    for _ in range(_batch_limit(limit)):
        ...
        processed += 1
    return TelegramNotificationOutboxQueueFeederReport(
        processed_count=processed,
        disposition_counts=counts,
    )


async def telegram_notification_outbox_queue_handoff_loop() -> None:
    assert_background_job_authority(JOB_TELEGRAM_DELIVERY_QUEUE)
    _assert_queue_owner()
    iteration = 0
    while True:
        ...
        with job_context(JOB_TELEGRAM_DELIVERY_QUEUE, iteration=iteration) as run_id:
            try:
                report = await run_telegram_notification_outbox_queue_handoff_cycle()
                if report.processed_count:
                    logger.info(
                        "Telegram notification outbox handoff cycle completed",
                        extra={
                            "event": "telegram_notification_outbox_queue_feeder.completed",
                ...
                raise
            except TelegramNotificationOutboxQueueFeederOwnershipError:
                raise
            except Exception as exc:
                _loop_errors.log(
                    logger,
                    "Error in Telegram notification outbox queue feeder: %s",
                    exc,
                    job_name=JOB_TELEGRAM_DELIVERY_QUEUE,
                    run_id=run_id,
                    iteration=iteration,
