...
@runtime_checkable
class PlannedObjectDeltaChange(Protocol):
    """The minimum safe shape of one already-validated database change."""

    logical_sequence: int


@runtime_checkable
class AtomicObjectDeltaImportPlan(Protocol):
    """Structural view of a pure import decision from the planning layer."""
...
# This is a review checklist for the future database adapter, not an API for
# adding behaviour.  It deliberately contains no notification, transport, or
# callback operation.
...
def _validate_apply_plan(plan: object) -> tuple[str, tuple[PlannedObjectDeltaChange, ...], object | None, object | None]:
    if not isinstance(plan, AtomicObjectDeltaImportPlan):
    ...
    for change in changes:
        if not isinstance(change, PlannedObjectDeltaChange):
