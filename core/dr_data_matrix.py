...
from core.dr_data_policy import (
    WEBAPP_DR_DROPPED_FIELDS,
    WEBAPP_DR_REPLICA_TABLES,
    WEBAPP_DR_REQUIRED_FIELDS,
)
...
def _field_classification(table_name: str, field_name: str, replication_class: str) -> dict[str, Any]:
    ...
    if replication_class in {"site_local", "internal_control"}:
        ...
        source = "table_policy"
    elif field_name in WEBAPP_DR_REQUIRED_FIELDS.get(table_name, frozenset()):
        classification = SyncFieldClassification.SYNC.value
        action = SyncFieldAction.KEEP.value
        reason = "required only on the authenticated private WebApp DR stream"
        source = "webapp_dr_required_field"
    ...
    return {
        ...
        "output_field": (
            None
            if source == "webapp_dr_required_field"
            else explicit.output_field if explicit else None
        ),
