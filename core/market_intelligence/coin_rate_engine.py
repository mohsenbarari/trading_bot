...
COIN_RATE_ENGINE_VERSION = "coin-rate-engine-v1"
...
def _tolerance(*, point: MeltedPoint, anchor_age: float | None, regime: str, structural_only: bool) -> tuple[float, float]:
    ...
    lower = min(0.018, base + source_extra + age_extra + (directional if regime == "DOWN" else 0.0))
    upper = min(0.018, base + source_extra + age_extra + (directional if regime == "UP" else 0.0))
    if regime == "VOLATILE":
        lower = upper = min(0.02, base + source_extra + age_extra + directional)
    return lower, upper
    ...
    for settlement in _SETTLEMENTS:
        ...
        for code, (coefficient, low_date) in COIN_SPECS.items():
            ...
            if estimate is None:
                ...
                continue
            negative, positive = _tolerance(point=current, anchor_age=anchor_age, regime=regime, structural_only=structural_only)
            ...
            output.append(CoinRateEstimate(code, settlement, "ESTIMATED", center, lower, upper, confidence, method, current.source_kind, anchor_age, regime))
    return output
