...
EXTRACTOR_VERSION = "reply-rules-v7-linked-reply-price-guard"
MAX_REPLY_AGE_SECONDS = 2 * 60 * 60
POST_CONFIRMATION_PRICE_AUDIT_SECONDS = 15 * 60
# A reply may negotiate a small change against a linked offer, but it must
# never replace that offer with a price from another market regime.  This is
# deliberately an absolute bound because coin prices in this parser are
# represented in project units (thousand tomans).
MAX_LINKED_REPLY_PRICE_DELTA = 3_000
...
def reply_price_adjustment(
    ...
    A short number in a reply is a *tail* of the linked offer price, never an
    independent market price.  For example, against ``182300`` the reply
    ``220 برکت`` means ``182220``.  Generic shorthand expansion would turn
    that text into ``220000`` and can silently poison both a trade label and
    ...
    if len(candidates) == 1:
        ...
        if abs(reconstructed - anchor) <= MAX_LINKED_REPLY_PRICE_DELTA:
            tail_result = {
                "price": reconstructed,
                "price_raw": token.raw,
    ...
    if choice is not None and tail_result is not None:
        ...
        if abs(int(choice.value) - anchor) <= abs(int(tail_result["price"]) - anchor):
            tail_result = None
    # ``choose_price`` is intentionally permissive for root offers, where a
    # 2/3-digit shorthand can be a standalone price.  It is unsafe in a reply:
    # a linked negotiation must either use the contextual tail above, or state
    # a complete nearby price.  In particular, never accept expanded shorthand
    # such as 220 -> 220000 here.
    if choice is not None and tail_result is None:
        if choice.method == "expanded_shorthand":
            return None
        if abs(int(choice.value) - int(offer["price"])) > MAX_LINKED_REPLY_PRICE_DELTA:
            return None
        return {
...
def analyze_reply_trades(
    ...
    for request_id, request in requests.items():
        ...
        if abs(int(price) - int(offer["price"])) > MAX_LINKED_REPLY_PRICE_DELTA:
            # Defense in depth: even if a future parser path produces an
            # adjustment, a linked trade that jumps outside its local market
            # anchor can neither be accepted nor used for training.
            reason = "REPLY_PRICE_OUTSIDE_LINKED_MARKET_RANGE"
            training_eligible = False
            confidence = min(confidence, 0.49)
            evidence.append("PRICE_OUTSIDE_LINKED_OFFER_RANGE")
