...
The acceptance runner invokes this with ``docker exec`` (over SSH for Iran). It
refuses to run outside a staging environment, writes only rows carrying the
caller's run prefix, and retires them through ``delete_user_account`` so the peer
learns too. Telegram transport is tested by the separate B2B harness; this driver
never manually leases or directly dispatches queue jobs.
...
async def _scenario_channel_marker_run(
    ...
    *,
    ...
    if not (
        bool(getattr(settings, "telegram_multi_publisher_enabled", False))
        and bool(getattr(settings, "telegram_b2b_dispatch_enabled", False))
    ):
        raise DriverRefusal("OT-CHANNEL-MARKER requires the B2B publisher path")
    ...
    publisher_identity = str(pub_state.publisher_bot_identity or "")
    if publisher_identity not in TELEGRAM_PUBLISHER_IDENTITIES:
        raise DriverRefusal(
            "channel publication does not carry a B2B publisher identity"
        )
    ...
    overtime_owner_unchanged = (
        str(pub_state.publisher_bot_identity or "") == publisher_identity
        and int(pub_state.telegram_message_id or 0) == telegram_message_id
    )
    ...
    overtime_b2b_owned = bool(
        overtime_job is not None
        and overtime_command is not None
        and str(overtime_job.bot_identity) == publisher_identity
        and str(overtime_command.publisher_bot_identity) == publisher_identity
    )
    ...
    final_command = (
        (
            await session.execute(
                select(TelegramPublisherDispatchCommand).where(
                    TelegramPublisherDispatchCommand.job_id == int(final_job.id)
                )
            )
        ).scalar_one_or_none()
    ...
    final_b2b_owned = bool(
        final_job is not None
        and final_command is not None
        and str(final_job.bot_identity) == publisher_identity
        and str(final_command.publisher_bot_identity) == publisher_identity
    )
    ...
    return {
        ...
        "final_tail_delivery_jobs": int(final_job is not None),
        "passed": passed,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    except Exception as exc:  # noqa: BLE001 - evidence records type, never internals
        print(
            json.dumps(
