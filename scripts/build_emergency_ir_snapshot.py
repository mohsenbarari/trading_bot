...
def _inspect_source(*, runner: Callable[..., Any]) -> None:
    try:
        inspected = runner(
            [str(DOCKER_BINARY), "inspect", "-f", "{{.Config.Image}}", PRODUCTION_DB_CONTAINER],
            check=False,
            capture_output=True,
            text=True,
            timeout=30,
    ...
    if getattr(inspected, "returncode", 1) != 0 or str(getattr(inspected, "stdout", "")).strip() != EXPECTED_SOURCE_IMAGE:
        _fail("production database source does not match the Emergency snapshot contract")


def _run_dump(*, temporary: Path, runner: Callable[..., Any]) -> None:
    descriptor: int | None = None
    try:
        descriptor = os.open(
            temporary,
...
def _validate_archive(*, temporary: Path, runner: Callable[..., Any]) -> None:
    try:
...
def capture_snapshot(*, output: Path, runner: Callable[..., Any] = subprocess.run) -> SnapshotResult:
    ...
    _inspect_source(runner=runner)
    _run_dump(temporary=temporary, runner=runner)
    digest, size = _digest(temporary)
    _validate_archive(temporary=temporary, runner=runner)
    try:
