...
def _build_commands(
    *, sha: str, tree: str, local_root: Path, remote_root: Path,
    ...
    web_snapshot_root = Path(web_values["MARKET_PRODUCT_SNAPSHOT_ROOT"])
    bot_snapshot_root = Path(bot_values["MARKET_PRODUCT_SNAPSHOT_ROOT"])
    ...
    bot_args = ["--release-sha", sha, "--receiver-db", str(bot_receiver_state / "market-fact-receiver.sqlite3"), "--market-store-db", str(bot_data / "market-store/market-store.sqlite"), "--estimator-state-db", str(bot_estimator_state / "estimator-state.sqlite3"), "--snapshot", str(bot_snapshot_root / "latest-estimator-snapshot.json")]
    phases.append(_phase(PHASES[7], [
        ...
        _command("web", remote_root, "observe_production_private_primary.py", ["--role", "web", "--release-sha", sha, "--release-tree", tree, "--project", web_project, "--image-id", market_images["web"], "--snapshot", str(web_snapshot_root / "latest-estimator-snapshot.json"), "--receiver-db", str(web_receiver_state / "estimator-snapshot-receiver.sqlite3"), "--output", web_observation]),
        _command("local", remote_root, "observe_production_private_primary.py", ["--role", "bot", "--release-sha", sha, "--release-tree", tree, "--project", bot_project, "--image-id", market_images["bot"], "--snapshot", str(bot_snapshot_root / "latest-estimator-snapshot.json"), "--receiver-db", str(bot_receiver_state / "market-fact-receiver.sqlite3"), "--market-store-db", str(bot_data / "market-store/market-store.sqlite"), "--sender-db", str(bot_sender_state / "sender-state.sqlite3"), "--output", bot_observation]),
    ...
    outbox_common = ["--database", str(web_receiver_state / "estimator-snapshot-receiver.sqlite3"), "--snapshot-root", str(web_snapshot_root), "--publication-events", str(web_receiver_state / "snapshot-publication-events.jsonl"), "--lane", "PRIVATE_PRIMARY", "--expected-release-sha", sha, "--expected-release-tree", tree, "--receipt", outbox_receipt]
