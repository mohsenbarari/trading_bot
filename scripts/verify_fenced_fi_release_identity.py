...
def verify(
    *,
    ...
    identity = load_verified_fenced_fi_release_identity(
...
def load_verified_fenced_fi_release_identity(
    *,
    descriptor_path: Path,
    authority_path: Path,
    expected_identity_sha256: str | None = None,
):
    """Return a verified identity to a local root-owned admission gate.

    This remains read-only and non-authorizing.  It is intentionally separate
    from :func:`verify` so the fenced cutover preflight can compare the signed
    release fields with its independently observed local files before a
    Writer Witness term is acquired.
    """
