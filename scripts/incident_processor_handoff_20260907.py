"""Processor-only replay/scan hotfix; preserve data, parent authority and peers."""
import argparse
...
NEW = '921b45a045073dab91f14b56f4bb8fc7c9a25344'
OLD_IMAGE = 'sha256:1371fce0a9158a06482d77e211a0265df59aa18f08e4f8639edbad97efe8eaad'
NEW_IMAGE = 'sha256:70133301cfdebca98887e651bfba8a87f45da21580fb77df5088562255c47472'
PORTABLE = '9791fee4402ef466ba0d33fdaf4163cea7686937a80d121633fb84249b9ab8c3'
...
ROOT = Path('/srv/trading-bot/market-pipeline-releases/f3be9ae2c4e9df0da31abaa41e91f327eb9573a2')
...
OPS = Path('/srv/trading-bot/incident-recovery/20260907-processor-sparse-minutes')
OVERRIDE = OPS / 'processor-sparse-minutes.override.json'
EXPORT_LIMIT = '500'
...
def compose(new=False):
    ...
    args += ['-f','/srv/trading-bot/incident-recovery/20260907-processor-export-capacity/processor-export-capacity.override.json']
    if new:
        args += ['-f',str(OVERRIDE)]
    return args
...
def healthy(release,image,old_mounts,since):
    ...
    require(h.get('schema')=='market_processor/4.0' and h.get('mode')=='live'
            and h.get('shadow_only') is True,'processor_contract_drift')
    require(h.get('status')=='live-shadow-ready','processor_not_running')
    ...
    if release==NEW:
        require(h.get('adapter_version')=='capture-event-adapter-v11-idempotent-public-replay',
                'processor_adapter_drift')
        require(isinstance(counters.get('pending_market_messages'),int)
                and counters['pending_market_messages']>=0,'processor_backlog_metrics_missing')
    # Activation proves liveness and ownership, not completed catch-up.
    # The recorded backlog must still be drained and checked end-to-end.
    return {'observed_at_utc':utc(),'container_id':c['Id'],'restart_count':0,
            ...
            'pending_market_messages':counters.get('pending_market_messages'),
            'projection_status':h.get('projection_status'),
            'cycle_duration_seconds':counters.get('cycle_duration_seconds'),
            'restart_policy':expected_policy}

def wait_healthy(release,image,old_mounts,since):
    # First activation may build additive indexes over retained history.
    # Keep live freshness requirements; allow bounded startup work.
