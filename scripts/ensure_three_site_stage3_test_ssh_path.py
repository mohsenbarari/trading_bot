...
RULE_DESCRIPTION = "stage3-test-bot-fi-ssh-jump"
...
def matching_rule(group: dict[str, Any]) -> dict[str, Any] | None:
    matches = [
        rule
        for rule in group.get("rules", [])
        if isinstance(rule, dict) and rule.get("description") == RULE_DESCRIPTION
    ]
    if len(matches) > 1:
