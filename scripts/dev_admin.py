...
async def forward_remote_session_reset(user: User, target_server: str) -> tuple[int, dict]:
    try:
        # A session reset used to be signed and POSTed to the peer.  There is
        # deliberately no direct FI<->IR command route in the physical
        # topology, so refuse before examining either local user data or a
        # peer target.
        assert_legacy_direct_fi_ir_transport_retired(
            component="dev-admin",
    ...
    # Keep the post-fence shape explicit for type checkers and for any future
    # refactor that changes the retirement assertion implementation.
