...
PREPARED_SCHEMA = "three-site-staging-witness-relay-prepared-v1"
FINAL_SCHEMA = "three-site-staging-witness-relay-final-v1"
...
SESSION_NAME = "human-approval-session.json"
POLICY_NAME = "human-approval-policy.json"
PREPARED_FILE_MODES = {
    COMPOSE_NAME: 0o640,
    ENV_NAME: 0o600,
    MANIFEST_NAME: 0o600,
...
SESSION_ROOT = Path("/var/lib/trading-bot/human-approvals")
...
def _relay_document_path(
    value: str, *, label: str, revision_id: str | None = None
) -> str:
    path = _normalized_absolute(value, label=label)
    if path == SESSION_ROOT or SESSION_ROOT not in path.parents:
        raise WitnessRelayMaterialError(
            f"{label} must be a unique child of {SESSION_ROOT}"
        )
    if path.name in {"session.json", "policy.json"}:
        raise WitnessRelayMaterialError(f"{label} must use a revision-unique file name")
    if revision_id is not None and revision_id not in path.name:
        raise WitnessRelayMaterialError(
            f"{label} file name must contain the relay revision ID"
        )
    return str(path)
...
def _required_actions(
    ...
    *,
    ...
    actions = tuple(sorted(set(values or REQUIRED_MATRIX_ACTIONS)))
    if not actions or any(not isinstance(action, str) or not action for action in actions):
        raise WitnessRelayMaterialError("required relay actions are invalid")
...
def _derive_environment(
    *,
    ...
    disabled = (
        ...
        base_values.get("STAGING_HUMAN_APPROVAL_RELAY_SESSION_FILE"),
        base_values.get("STAGING_HUMAN_APPROVAL_RELAY_POLICY_FILE"),
    ...
    if disabled != ("false", "/dev/null", "/dev/null", "", ""):
        raise WitnessRelayMaterialError(
    ...
    if base_values.get("STAGING_SOURCE_ROOT") == exact_source_root:
        raise WitnessRelayMaterialError(
            "base source root already equals the immutable release; six changes required"
        )
    ...
    revised_values.update(
        {
            ...
            "STAGING_HUMAN_APPROVAL_RELAY_SESSION_FILE": session_file,
            "STAGING_HUMAN_APPROVAL_RELAY_POLICY_FILE": policy_file,
    ...
    if changed != ALLOWED_ENV_CHANGES:
        raise WitnessRelayMaterialError(
            "relay revision does not change exactly the six allowed variables"
        )
...
def derive_prepared_revision(
    *,
    ...
    session_path = _relay_document_path(
        session_file, label="relay session path", revision_id=revision
    )
    policy_path = _relay_document_path(
        policy_file, label="relay policy path", revision_id=revision
    )
    if session_path == policy_path:
        raise WitnessRelayMaterialError("relay session and policy paths must differ")
    actions = _required_actions(required_actions, policy=approval_policy)
...
def verify_prepared_structure(
    *,
    ...
    """Verify the six-field revision without file or image attestation."""
    ...
    session_file = _relay_document_path(
        str(bindings["session_file"]),
        label="relay session path",
        revision_id=revision,
    )
    policy_file = _relay_document_path(
        str(bindings["policy_file"]),
        label="relay policy path",
        revision_id=revision,
    )
    if session_file == policy_file:
        raise WitnessRelayMaterialError("relay session and policy paths must differ")
    ...
    if changed != ALLOWED_ENV_CHANGES or disabled != (
        ...
        raise WitnessRelayMaterialError(
            "prepared relay material is not an exact six-field disabled-baseline revision"
        )
    expected_values = {
    ...
    actions = _required_actions(
        list(manifest["required_session_actions"]), policy=approval_policy
    )
...
def validate_prepared_campaign(
    *,
    ...
    if approval.get("schema") == RELAY_RECEIPT_SCHEMA:
        raise WitnessRelayMaterialError(
            "prepared campaign validation refuses relay receipts until the "
            "release verifier accepts an explicit campaign-bound Witness key"
        )
...
def _read_final_directory(
    ...
    _assert_existing_bundle_directory(directory)
    if set(path.name for path in directory.iterdir()) != set(FINAL_FILE_MODES):
...
def main(argv: list[str] | None = None) -> int:
    ...
    derive.add_argument("--session-file", required=True)
    derive.add_argument("--policy-file", required=True)
    derive.add_argument("--relay-key-id", required=True)
    derive.add_argument("--relay-secret-file", type=Path, required=True)
    derive.add_argument("--required-action", action="append")
