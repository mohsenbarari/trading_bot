...
def _row_target_key(row: sqlite3.Row) -> tuple[str, bool] | None:
    ...
    if instrument in {"MELTED_GOLD_FLOW", "MELTED_GOLD_PRIVATE"}:
        key, label_conditional = market_key_from_fields(
        ...
        conditional = bool(row["is_conditional"]) or label_conditional
        if conditional and key.endswith(":GENERIC"):
            key = key.removesuffix(":GENERIC") + ":CONDITIONAL"
        # Private order flow is the preferred canonical anchor. Public flow
        # remains a separate feature family and can never be averaged into it.
        if instrument == "MELTED_GOLD_FLOW":
            key = f"PUBLIC_FLOW:{key}"
        return key, conditional
    if instrument == "COIN_PUBLIC_CHANNEL":
        trade_form = str(row["trade_form"] or "UNKNOWN").upper()
        if trade_form not in {"PAPER_NORMAL", "PAPER_REVERSE", "PAPER_SWIM", "PHYSICAL"}:
            return None
        ...
        return f"COIN_REFERENCE:{trade_form}", bool(row["is_conditional"])
    return None
...
def _support_key(row: sqlite3.Row) -> tuple[str, bool] | None:
    ...
    if instrument == "MELTED_GOLD_UNION":
        return "GOLD_UNION_QUOTE", conditional
    if instrument in {"XAUUSD", "AED_DUBAI"}:
...
def load_canonical_market_data(
    ...
    *,
    ...
    if not database.exists():
        raise ValueError("canonical_market_store_not_found")
    connection = connect_market_store_read_only(database)
    verify_market_store_read_only(connection)
    query = """
        WITH eligible AS (
            SELECT id, instrument, market_label, settlement_term, trade_form,
                   event_type, side, price_num, quantity_num, available_at_utc,
                   is_conditional
        ...
        SELECT instrument, market_label, settlement_term, trade_form, event_type,
               side, price_num, quantity_num, available_at_utc, is_conditional
        FROM eligible
        WHERE instrument IN (
            'MELTED_GOLD_FLOW', 'MELTED_GOLD_PRIVATE', 'COIN_PUBLIC_CHANNEL',
            'MELTED_GOLD_AGGREGATE', 'USD_HERAT', 'XAUUSD',
            'MELTED_GOLD_UNION', 'AED_DUBAI'
        )
          AND price_num IS NOT NULL AND price_num > 0
          AND available_at_utc IS NOT NULL
        ORDER BY available_at_utc ASC, id ASC
    """
    ...
    try:
        for row in connection.execute(query):
            try:
                # Research availability, rather than source event time, is the
                # only admissible feature timestamp. Late data cannot travel
                # backward into an earlier cutoff.
                observed_at = _parse_utc(str(row["available_at_utc"]))
            ...
            elif instrument == "COIN_PUBLIC_CHANNEL" and include_coin_reference:
                target_info = _row_target_key(row)
                if target_info is None:
                    discarded["unsupported"] += 1
                    continue
            ...
            elif instrument == "COIN_PUBLIC_CHANNEL":
...
def load_canonical_confirmed_coin_trade_targets(
    market_store: Path,
    *,
    since_utc: datetime | None,
    until_utc: datetime | None,
) -> tuple[dict[str, list[TargetPoint]], list[ConfirmedCoinTrade], dict[str, object]]:
    """Load only eligible, non-conditional confirmed trades from Market Store.

    Offers never become outcome labels. ``available_at_utc`` is used as the
    research timestamp, so a delayed confirmation cannot leak into an earlier
    feature cutoff.
    """

    connection = connect_market_store_read_only(market_store)
    verify_market_store_read_only(connection)
    query = """
        WITH eligible AS (
            SELECT id, instrument, settlement_term, trade_form, event_type,
                   price_num, price_unit, available_at_utc, is_conditional
            FROM market_observations
            WHERE quality_state = 'ELIGIBLE'
            UNION ALL
            SELECT id, instrument, settlement_term, trade_form, event_type,
        ...
        SELECT instrument, settlement_term, trade_form, price_num,
               available_at_utc
        FROM eligible
    ...
    try:
        for row in connection.execute(query):
            ...
            if price is None or price <= 0:
                discarded += 1
                continue
            if since_utc is not None and available_at < since_utc:
                continue
            if until_utc is not None and available_at > until_utc:
                continue
            ...
            targets.setdefault(key, []).append(
                TargetPoint(key, available_at, price)
            )
            trades.append(
                ConfirmedCoinTrade(
                    occurred_at_utc=available_at,
                    commodity=commodity,
                    settlement=settlement,
                    trade_form=trade_form,
                    project_price=price,
    ...
    return targets, trades, {
        "enabled": True,
        "source": "canonical_market_store_confirmed_trades",
        "target_rows": len(trades),
        "discarded_rows": discarded,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Canonical Market Store v3; opened read-only and schema-verified.",
    )
    parser.add_argument(
        "--coin-trade-target-db",
        type=Path,
        help=(
            "Optional supplemental normalized confirmed-trade sidecar; canonical "
            "Market Store TRADE rows remain the primary labels."
        ),
    )
...
def main() -> int:
    ...
    market_store_path = _outside_repository(args.market_store)
    coin_trade_target_path = (
        _outside_repository(args.coin_trade_target_db)
        if args.coin_trade_target_db is not None
        else None
    )
    melted, support, targets, discarded = load_canonical_market_data(
        market_store_path,
    ...
    canonical_targets, coin_trades, canonical_trade_input = (
    ...
    for key, values in canonical_targets.items():
        targets.setdefault(key, []).extend(values)
    supplemental_trade_input: dict[str, object] = {"enabled": False}
    if coin_trade_target_path is not None:
        _, supplemental_trades, supplemental_trade_input = (
            load_confirmed_coin_trade_targets(
                coin_trade_target_path,
                since_utc=args.since,
                until_utc=args.until,
            )
        ...
        known = {
            ...
            for item in coin_trades
        }
        for item in supplemental_trades:
            identity = (
                item.occurred_at_utc,
                item.commodity,
                item.settlement,
            ...
            if identity in known:
                continue
            known.add(identity)
            coin_trades.append(item)
            key = (
                f"COIN_TRADE:{item.commodity}:{item.settlement}:{item.trade_form}"
            )
            targets.setdefault(key, []).append(
                TargetPoint(key, item.occurred_at_utc, item.project_price)
            )
    dataset_handle = None
    dataset_temp = None
    ...
    try:
        ...
        report["input"] = {
            "market_store_contract": "canonical_v3_read_only",
            "research_timestamp": "available_at_utc",
            "since_utc": _iso_utc(args.since) if args.since else None,
            "until_utc": _iso_utc(args.until) if args.until else None,
            "discarded_rows": discarded,
            "canonical_confirmed_coin_trade_targets": canonical_trade_input,
            "supplemental_confirmed_coin_trade_targets": supplemental_trade_input,
        }
