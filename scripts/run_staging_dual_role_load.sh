...
case "${1:-}" in
    -h|--help|help)
        printf '%s\n' 'Legacy two-server staging dual-role load is retired. Only artifact-only planning in the sealed three-site campaign is supported.'
        exit 0
        ;;
    *)
        printf '%s\n' 'Legacy two-server staging dual-role load is retired and hard-disabled before Docker, HTTP, or data access. Use the sealed three-site campaign verifier.' >&2
        exit 2
        ;;
esac
